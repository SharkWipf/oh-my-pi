...
const user = (entryId: string, order: number, content: string): SourceMessage<Message> => ({
	entryId,
	order,
	message: { role: "user", content, timestamp: order },
});
...
describe("chronological source archive", () => {
	it("unions one long entry across a split normalization unit and preserves the map after reload", async () => {
		...
		for (const raw of ["A".repeat(57) + "→" + "B".repeat(250), "A".repeat(257) + "→" + "B".repeat(63)]) {
			...
			const repeated = await snap.compact(
				{
					...prepare([], [source]),
					previousSummary: ordinary.summary,
					previousPreserveData: JSON.parse(JSON.stringify(ordinary.preserveData)),
				},
				options,
			);
			for (const result of [selected, repeated]) {
				...
				for (const run of representation.coverage) {
					...
					expect(archive.text!.slice(run.normalized!.start, run.normalized!.end)).toBe(
						unit ? normalized.slice(unit.start, unit.end) : normalized,
					);
				}
	...
	it("restores complete admitted tool content without replacing historical source bytes", async () => {
		...
		const assistant: SourceMessage<Message> = {
			entryId: "a",
			order: 0,
			atomicGroup,
			message: {
		...
		const result: SourceMessage<Message> = {
			entryId: "r",
			order: 1,
			atomicGroup,
			message: {
		...
		for (const run of ordinary.representation.coverage)
			if (run.current && run.current.end > run.current.start) {
				expect(
					selected.representation.coverage.some(
						next =>
		...
		const prior = {
			...selectedResult.preserveData,
			sourceRepresentation: {
				...selected.representation,
				coverage: selected.representation.coverage.map(run => ({
					...run,
					current: undefined,
					status: "historical-not-current" as const,
				})),
			},
		};
		const current: SourceMessage<Message> = {
			...result,
			message: {
				...(result.message as Extract<Message, { role: "toolResult" }>),
				content: [{ type: "text", text: "CURRENT REPLACEMENT" }],
			},
		};
		const repeated = unpack(
			await snap.compact(
				{ ...prepare([], [assistant, current]), previousPreserveData: prior },
				{ shape, maxFrames: 1 },
			),
		);
		const baseline = unpack(
			await snap.compact({ ...prepare([], []), previousPreserveData: prior }, { shape, maxFrames: 1 }),
		);
		...
		expect(
			repeated.representation.coverage.some(run => run.entryId === "r" && run.status === "historical-not-current"),
		).toBe(true);
	});

	it("retains original raw offsets through sparse selection and Unicode expansion on the next compaction", async () => {
		const source = {
			...user("sparse", 0, "AB→CDXYZ"),
			spans: [
				{ blockIndex: 0, start: 0, end: 3 },
				{ blockIndex: 0, start: 6, end: 8 },
			],
		};
		...
		expect(coverage.map(run => run.current)).toEqual([
			...
			{ blockIndex: 0, start: 2, end: 3 },
			{ blockIndex: 0, start: 6, end: 8 },
		]);
		const sameText = user("different-id", 1, "AB→CDXYZ");
		const repeated = unpack(
			await snap.compact({ ...prepare([sameText], []), previousPreserveData: first.preserveData }, { shape }),
		);
		expect(repeated.archive.text).toContain("AB->[truncated]YZ");
		expect(repeated.archive.text).toContain("AB->CDXYZ");
		expect(new Set(repeated.representation.coverage.map(run => run.entryId))).toEqual(
			new Set(["sparse", "different-id"]),
		);
	});
	...
	it("attributes an added original image independently of ordinary text and resets its next-archive ownership", async () => {
		const image: ImageContent = {
			type: "image",
			data: (await snap.render("OWNED SOURCE IMAGE", shape)).data,
			mimeType: "image/png",
		};
		const source: SourceMessage<Message> = {
			...
			message: {
				role: "user",
				timestamp: 0,
				content: [{ type: "text", text: "ordinary before" }, image, { type: "text", text: "ordinary after" }],
			},
		};
		...
		expect(frozen.representation.coverage.filter(run => run.normalized).map(run => run.contribution)).toEqual([
			"ordinary",
			"ordinary",
		]);
		const imageRuns = frozen.representation.coverage.filter(run => !run.normalized);
		expect(imageRuns).toEqual([
			{
				entryId: "mixed",
				order: 0,
				snapshot: { blockIndex: 1, start: 0, end: 1 },
				current: { blockIndex: 1, start: 0, end: 1 },
				status: "exact-current",
				contribution: "selected-user",
			},
		]);
		const repeated = unpack(
			await snap.compact({ ...prepare([], [selected]), previousPreserveData: first.preserveData }, { shape }),
		);
		expect(repeated.representation.coverage.filter(run => !run.normalized)).toEqual(
			imageRuns.map(run => ({ ...run, contribution: "ordinary" })),
		);
		for (const archive of [frozen, repeated]) {
			expect(
				snap
					.historyBlocks(archive.archive, {
						sourceRepresentation: archive.representation,
	...
	it("keeps a tool-result original image inside the standard folded atomic conversation", async () => {
		...
		const assistant: SourceMessage<Message> = {
			...
			message: {
				...
				content: [
					...
					{ type: "text", text: "ASSISTANT AFTER" },
				],
			},
		};
		const image: ImageContent = {
			type: "image",
			data: (await snap.render("SOURCE IMAGE", shape)).data,
			mimeType: "image/png",
		};
		const result: SourceMessage<Message> = {
			entryId: "result",
		...
		const blocks = snap.historyBlocks(emitted.archive, {
			sourceRepresentation: emitted.representation,
			resolveSourceImage: () => image,
		});
		...
		expect(
			emitted.representation.coverage
				.filter(run => run.entryId === "result" && run.normalized)
				.map(run => run.current?.blockIndex),
		).toEqual([0, 2]);
		expect(emitted.representation.coverage.filter(run => run.entryId === "result" && !run.normalized)).toEqual([
			{
				entryId: "result",
				order: 1,
				atomicGroup,
	...
	it("counts only wholly absent source IDs and does not call raster overflow an omission", async () => {
		...
		const ordinary = unpack(
			await snap.compact(prepare(sources, []), { shape: { ...shape, frameSize: 64 }, maxFrames: 1 }),
		);
		...
		expect(
			ordinary.representation.layout.reduce(
				(sum, part) => sum + (part.kind === "gap" ? (part.wholeMessages ?? 0) : 0),
				0,
			),
		).toBe(absent);
		const selected = unpack(
			await snap.compact(prepare(sources), { shape: { ...shape, frameSize: 64 }, maxFrames: 1 }),
		);
		...
		expect(
			snap
				.historyBlocks(selected.archive, { sourceRepresentation: selected.representation })
				.some(block => block.type === "text" && block.text.startsWith("[continued]")),
		).toBe(true);
	});
	it("removes a closed sparse gap without dropping fuller ordinary spans", async () => {
		...
		const sparse = {
			...
			spans: [
				{ blockIndex: 0, start: 0, end: 2 },
				{ blockIndex: 0, start: 8, end: 10 },
			],
		};
		const first = await snap.compact(prepare([sparse], []), { shape });
		const filled = unpack(
			await snap.compact({ ...prepare([], [source]), previousPreserveData: first.preserveData }, { shape }),
		);
	...
	it("unions selected intervals into a partially retained recent source", async () => {
		...
		const result = unpack(
			await snap.compact(
				{ ...prepare([user("old", 0, "history")], [selected]), recentSources: [ordinary] },
				{ shape },
			),
		);
		expect(
			result.representation.layout.find(part => part.kind === "source" && part.entryId === "recent"),
		).toMatchObject({ spans: [{ blockIndex: 0, start: 0, end: 10 }] });
	});
	...
	it("keeps vanilla pagination with zero selection and reduces reconstructed rescue history", async () => {
		...
		const {
			sourcesToSummarize: _sources,
			turnPrefixSources: _prefix,
			recentSources: _recent,
			selectedSources: _selected,
			...plain
		} = mapped;
		...
		const selected = await snap.compact(prepare([source]), options);
		const rescue = unpack(
			await snap.compact({ ...prepare([], []), previousPreserveData: selected.preserveData }, options),
		);
		const physical = (result: ReturnType<typeof unpack>) =>
			snap
				.historyBlocks(result.archive, { sourceRepresentation: result.representation })
				.reduce((sum, block) => sum + (block.type === "text" ? block.text.length : 100), 0);
...
it("rebuilds serialized source frames lazily without spending original-image or later-frame bytes", async () => {
	const image: ImageContent = {
		type: "image",
		data: (await snap.render("ORIGINAL", shape)).data,
		mimeType: "image/png",
	};
	...
	for (const result of [first, repeated, cleared]) {
		...
		const external = {
			...archive,
			frames: archive.frames.map((frame, i) => ({ ...frame, data: `blob:sha256:${i}` })),
		};
		const reads: number[] = [];
		...
		const expected = snap.historyBlocks(archive, {
			sourceRepresentation: representation,
			maxFrameDataBytes: budget,
			resolveSourceImage: () => image,
		});
		const blocks = snap.historyBlocks(external, {
			sourceRepresentation: representation,
			maxFrameDataBytes: budget,
			resolveFrameData,
			resolveSourceImage: () => image,
		...
		expect(blocks.filter(block => block.type === "image").map(block => block.data)).toEqual([
			payloads[0],
			image.data,
		]);
		const replayedText: string[] = [];
		const zero = snap.historyBlocks(external, {
			sourceRepresentation: representation,
			maxFrameDataBytes: 0,
			resolveFrameData,
			resolveSourceImage: () => image,
			onEmit: (i, _blockIndex, block) => {
				if (representation.layout[i]!.kind !== "gap" && block.type === "text") replayedText.push(block.text);
			},
		});
		expect(reads).toEqual([0]);
		...
		expect(
			snap.historyBlocks(external, {
				sourceRepresentation: representation,
				resolveFrameData: () => undefined,
				resolveSourceImage: () => image,
			}),
		).toEqual(zero);
		reads.length = 0;
		const legacy = snap.historyBlocks(
			{ frames: external.frames, totalChars: external.totalChars, truncatedChars: 0 },
			{ maxFrameDataBytes: payloads[2]!.length, resolveFrameData },
		);
