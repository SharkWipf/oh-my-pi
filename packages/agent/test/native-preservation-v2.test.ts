...
} from "../src/compaction/compaction-v2-streaming";
	...
	return setSourceOrigin(
		{ role: "user", content: marker + text },
		{
			...
			parts: origin.parts.map(part => ({
				...
				transportSpan: { start: marker.length, end: marker.length + text.length },
			})),
		},
	);
}
function sourceIds(items: Record<string, unknown>[]): string[] {
	return items.flatMap(item => {
		...
		return origin?.kind === "source" ? [origin.parts[0].entryId] : [];
	});
}
...
describe("native V2 protected source union", () => {
	test("native retained delivery and selected original do not share a source slot", () => {
		...
		const result = buildCompactionV2ReplacementHistory([delivered], compaction(), 100, {
			userCandidates: [original],
			nonUserItems: [],
			nonUserSourceIds: [],
			nonUserTokens: 0,
		});
	...
	test("P does not refund overlapping ordinary sources or change its cut; equal text IDs remain distinct", () => {
		...
		const selected = buildCompactionV2ReplacementHistory(items, compaction(), 1, {
			userCandidates: [items[0], items[2]],
			nonUserItems: [],
			nonUserSourceIds: [],
			nonUserTokens: 0,
		}).replacementHistory;
	...
	test("actual budget-one string prefix and independent tail candidate share one source slot", () => {
		...
		const result = buildCompactionV2ReplacementHistory([original], compaction(), 1, {
			userCandidates: [tail],
			nonUserItems: [],
			nonUserSourceIds: [],
			nonUserTokens: 0,
		...
		expect(origin?.kind === "source" && origin.parts.map(part => part.sourceSpan)).toEqual([
			{ start: 0, end: 4 },
			{ start: 132, end: 200 },
		]);
	...
	test("complete multi-tool N is precharged before allocation, with zero-residual native minimum retained", () => {
		const source = (entryId: string, order: number, blockIndex: number): NativeSourcePart => ({
			entryId,
			order,
			blockIndex,
			coverage: "full",
		...
		const atom = [
			setSourceOrigin(
				{ type: "function_call", call_id: "c1", name: "read", arguments: '{"path":"one"}' },
				{ kind: "source", parts: [source("assistant", 0, 0)] },
			),
			setSourceOrigin(
				{ type: "function_call", call_id: "c2", name: "read", arguments: '{"path":"two"}' },
				{ kind: "source", parts: [source("assistant", 0, 1)] },
			),
			setSourceOrigin(
				{ type: "function_call_output", call_id: "c1", output: "full first output" },
				{ kind: "source", parts: [source("tool1", 1, 0)] },
			),
			setSourceOrigin(
				{ type: "function_call_output", call_id: "c2", output: "full second output" },
				{ kind: "source", parts: [source("tool2", 2, 0)] },
			),
		];
		const older = user("older", 3, "old!");
		const newest = user("newest", 4, "new!more");
		const result = buildCompactionV2ReplacementHistory([...atom, older, newest], compaction(), 2, {
			userCandidates: [],
			nonUserSourceIds: ["assistant", "tool1", "tool2"],
			nonUserItems: atom,
			nonUserTokens: 40,
		});
		...
		expect(
			result.replacementHistory.filter(item => item.type === "function_call_output").map(item => item.output),
		).toEqual(["full first output", "full second output"]);
	...
	test("original image survives partial ordinary retention once and reload retains exact text/image origins", async () => {
		...
		const result = buildCompactionV2ReplacementHistory(input, compaction(), 1, {
			userCandidates: input,
			nonUserItems: [],
			nonUserSourceIds: [],
			nonUserTokens: 0,
		});
		...
		const data = JSON.parse(
			JSON.stringify(
				storeCompactionV2PreserveData(
					{ ...result, compactionItem: result.replacementHistory.at(-1)!, usedTokens: 20 },
					model,
				),
			),
		);
	...
	test("provider-boundary stream keeps local selections off wire and returns frozen ordinary-plus-P union", async () => {
		...
		const request = buildCompactionV2RequestFromBody(
			model,
			{ model: model.id, input, stream: true },
			{
				retainedMessageBudget: 1,
				preservation: { userCandidates: [older], nonUserItems: [], nonUserSourceIds: [], nonUserTokens: 0 },
			},
		);
		const response = await requestCompactionV2Streaming(model, "fixture-key", request, undefined, {
		...
		try {
			...
			for (const generated of [false, true]) {
				const assistant: AssistantMessage = {
					...
					model: model.id,
					usage,
					stopReason: generated ? "stop" : "toolUse",
					timestamp: 1,
					content: generated
						? []
						: [
								...
								providerPayload: {
									...
									items: [
										...
										},
										{
											...
											status: "completed",
											container_id: "local",
											code: "print('logical-code')",
											outputs: [{ type: "logs" as const, logs: "logical execution output" }],
										},
									],
								},
							}
				...
				const screenshot: ToolResultMessage = {
					...
					providerMetadata: {
						type: "computer",
						screenshot: { type: "computer_screenshot", image_url: `data:image/png;base64,${png}` },
				...
				const ordinary = ["excluded", "m".repeat(8000), "n".repeat(4000)].map((content, index) => ({
					entryId: ["old", "middle", "newest"][index],
					order: index + 2,
					message: { role: "user" as const, content, timestamp: index + 3 },
				}));
				let baseline: Array<Record<string, unknown>> = [];
				for (const inflated of [false, true])
					for (const overlap of [false, true]) {
						const preparation: CompactionPreparation = {
							firstKeptEntryId: "newest",
							messagesToSummarize: [...originals, ...ordinary.slice(0, 2).map(source => source.message)],
							turnPrefixMessages: [],
							recentMessages: [ordinary[2].message],
							isSplitTurn: false,
							tokensBefore: 10000,
							fileOps: createFileOps(),
							settings: { ...DEFAULT_COMPACTION_SETTINGS, v2RetainedMessageBudget: 3000 },
							sourcesToSummarize: [...sources, ...ordinary.slice(0, 2)],
							recentSources: [ordinary[2]],
							selectedSources: [...sources, ...sources, ...(overlap ? [ordinary[2]] : [])],
						};
						const result = await compact(preparation, model, "localhost-key", undefined, undefined, {
							remoteSystemPrompt: ["local source quote fixture"],
							convertToLlm(messages) {
								if (inflated)
									for (const message of messages)
										if (message.role === "assistant" || message.role === "toolResult")
											message.content.push({ type: "text", text: "conversion inflation ".repeat(1000) });
								return cloneWithSourceOrigins(messages) as Message[];
							},
						});
						if (inflated)
							for (const message of originals)
								if (message.role === "assistant" || message.role === "toolResult") message.content.pop();
						expect(result.retentionTarget).toEqual({
							configuredTokens: 3000,
							manualNonUserTokens: quote,
							residualTokens: Math.max(1, 3000 - quote),
						});
						const history = getCompactionV2PreserveData(result.preserveData)!.replacementHistory;
						const users = history.filter(item =>
							sourceIds([item]).some(id => ["old", "middle", "newest"].includes(id)),
						);
						...
						expect(
							history.filter(item => item.type === (generated ? "image_generation_call" : "computer_call")),
						).toHaveLength(1);
						if (generated)
							expect(history.filter(item => item.type === "code_interpreter_call")).toMatchObject([
								{
									code: "print('logical-code')",
									outputs: [{ type: "logs", logs: "logical execution output" }],
								},
							]);
						else expect(history.filter(item => item.type === "computer_call_output")).toHaveLength(1);
						const imageItems = history
							.flatMap(item => (Array.isArray(item.content) ? item.content : []))
							.filter(block => block.type === "input_image");
						if (generated)
							expect(imageItems.map(block => block.image_url)).toEqual([`data:image/png;base64,${png}`]);
						else
							expect(history.find(item => item.type === "computer_call_output")?.output).toEqual({
...
test("Anthropic native summary retains the selected-source layout beside its encrypted aggregate", async () => {
	const model = buildModel({
		id: "claude-fable-5",
		name: "Claude Fable 5",
	...
	const preparation: CompactionPreparation = {
		firstKeptEntryId: "ordinary-tail",
		messagesToSummarize: [
			{
				role: "user",
				content: [
					{ type: "text", text: "selected original" },
					{ type: "image", mimeType: "image/png", data: "aW1hZ2U=" },
				],
				timestamp: 1,
			},
		...
		turnPrefixMessages: [],
		recentMessages: [{ role: "user", content: "ordinary tail", timestamp: 2 }],
		isSplitTurn: false,
		...
		sourcePreserveData: {
			sourceRepresentation: {
				version: 1,
				throughEntryId: "ordinary-tail",
				aggregate: { entryIds: ["selected-original"], reason: "summary" },
				coverage: [
					{
						entryId: "selected-original",
						order: 0,
						snapshot: { blockIndex: 0, start: 0, end: 17 },
						current: { blockIndex: 0, start: 0, end: 17 },
				...
				layout: [
					{
						...
						entryId: "selected-original",
						order: 0,
						spans: [{ blockIndex: 0, start: 0, end: 17 }],
						contribution: "selected-user",
					},
					{ kind: "original-image", entryId: "selected-original", order: 0, blockIndex: 1, currentBlockIndex: 1 },
					{ kind: "source", entryId: "ordinary-tail", order: 1, contribution: "ordinary" },
				],
			},
		},
	};
	const result = await compact(preparation, model, "fixture-key", undefined, undefined, {
		completeImpl: async () => ({
			role: "assistant",
			content: [],
			timestamp: 3,
			...
			providerPayload: {
				...
				provider: "anthropic",
				content: "Native aggregate summary.",
				encryptedContent: "opaque-aggregate",
			},
		}),
	});
	const persisted = JSON.parse(JSON.stringify(result.preserveData));
	expect(persisted.anthropicCompaction).toMatchObject({
		content: "Native aggregate summary.",
		encryptedContent: "opaque-aggregate",
	});
	expect(persisted.sourceRepresentation).toEqual(preparation.sourcePreserveData!.sourceRepresentation);
	expect(persisted.openaiRemoteCompaction).toBeUndefined();
});
