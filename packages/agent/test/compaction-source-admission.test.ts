...
test("repeated compaction retains the actual sparse source stream and unions partial ordinary coverage", () => {
	...
	const repeated = prepareCompaction(
		...
		{
			selectedSources: [
				{ entryId: "u2", order: 2, message: original, spans: [{ blockIndex: 0, start: 0, end: 2 }] },
			],
		},
	)!;
...
test("pending delivered users join selection without expanding the ordinary retained tail", () => {
	...
	const entries: SessionEntry[] = Array.from({ length: 8 }, (_, index) => ({
		...
		message: createUserMessage("Original " + index),
	}));
	...
	const pending = prepareCompaction(entries, settings, undefined, tokenizer, {
		pendingSourceEntryIds: new Set(["u0"]),
	})!;
	...
	const retained = pending.sourcePreserveData!.sourceRepresentation.layout.flatMap(part =>
		part.kind === "source" ? [part.entryId] : [],
	);
...
test("selected source intervals materialize the original submission rather than expanded delivery", () => {
	...
	const entries: SessionEntry[] = Array.from({ length: 4 }, (_, index) => ({
		...
		parentId: index ? "u" + (index - 1) : null,
		timestamp: "2026-09-08T00:00:00.000Z",
		message: createUserMessage(index === 0 ? "EXPANDED_LAST_BODY" : "tail " + index),
	}));
	const selection = {
		selectedSources: [{ entryId: "u0", order: 0, message: original, spans: [{ blockIndex: 0, start: 6, end: 28 }] }],
	};
	const settings = { ...DEFAULT_COMPACTION_SETTINGS, keepRecentTokens: 20 };
	...
	expect(materializeCompactionSourceMessage(source.message, source.spans)).toMatchObject({
		content: "[truncated]SELECTED_LAST_ORIGINAL",
	});
	const pending = prepareCompaction(entries, settings, undefined, tokenizer, {
		...selection,
		pendingSourceEntryIds: new Set(["u0"]),
	})!.selectedSources![0]!;
	expect(materializeCompactionSourceMessage(pending.message, pending.spans)).toMatchObject({
		content: "/keep SELECTED_LAST_ORIGINAL",
	});
});
...
test("ordinary delivery and selected original remain distinct adjacent projections across compaction", async () => {
	...
	const delivered = {
		role: "user" as const,
		content: "EXPANDED_LAST_BODY with longer ordinary context",
		timestamp: 0,
	};
	const entry = (id: string, message = createUserMessage(id)): SessionEntry => ({
		type: "message",
		id,
		parentId: null,
		timestamp: "2026-09-08T00:00:00.000Z",
	...
	const selection = {
		originalSourceMessage: () => original,
		selectedSources: [
			{
				entryId: "mixed",
				order: 1,
				...
				message: original,
				spans: [{ blockIndex: 0, start: 0, end: original.content.length }],
			},
		],
	};
	...
	const sourceParts = preparation.sourcePreserveData!.sourceRepresentation.layout.filter(
		part => part.kind === "source",
	);
	const materialized = sourceParts.map(part =>
		materializeCompactionSourceMessage(
			part.projection ? original : part.entryId === "mixed" ? delivered : createUserMessage(part.entryId),
			part.spans,
		),
	);
	expect(materialized.map(message => (message && "content" in message ? message.content : undefined))).toEqual([
		delivered.content,
		original.content,
		"tail",
	]);
	const result = await snap.compact(preparation);
	const representation = result.preserveData!.sourceRepresentation as NonNullable<
		typeof preparation.sourcePreserveData
	>["sourceRepresentation"];
	expect(
		representation.layout.filter(part => part.kind === "source").map(part => [part.entryId, part.projection]),
	).toEqual([
		["mixed", undefined],
		["mixed", "original"],
		["tail", undefined],
	...
	expect(
		repeated
			.sourcesToSummarize!.filter(source => source.entryId === "mixed")
			.map(source => [source.projection, "content" in source.message ? source.message.content : undefined]),
	).toEqual([
		[undefined, delivered.content],
	...
	const coverage = (archived.preserveData!.sourceRepresentation as typeof representation).coverage.filter(
		run => run.entryId === "mixed",
	);
	for (const run of coverage) {
		...
		expect(archive.text!.slice(run.normalized!.start, run.normalized!.end)).toBe(
			raw.slice(run.current!.start, run.current!.end),
		);
	}
});
