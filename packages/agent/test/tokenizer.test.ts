...
describe("Tokenizer", () => {
	test("counts each original image once across user, developer, tool, custom and hook content", () => {
		...
		const messages: AgentMessage[] = [
			...
			{ role: "custom", customType: "images", content, display: false, timestamp: 0 },
		];
		...
		expect(tokenizer.countMessages(messages)).toBe(5 * expected);
	});
	...
	test("charges normalized generated images once and never revives deleted native mirrors after JSON reload", () => {
		...
		const rewritten: AssistantMessage = JSON.parse(
			JSON.stringify({
				...message,
				content: [image, { ...text, text: "current rewritten text" }],
				providerPayload: {
					...payload,
		...
		expect(tokenizer.countMessage(rewritten)).toBe(1200 + tokenizer.countTokens("current rewritten text"));
