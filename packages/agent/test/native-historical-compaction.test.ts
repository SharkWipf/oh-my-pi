...
function fixture() {
	...
	const assistant: AssistantMessage = {
		...
		content: [
			...
			{
				type: "thinking",
				thinking: "full plaintext thinking",
				thinkingSignature: JSON.stringify({ type: "reasoning", encrypted_content: "opaque-thinking-signature" }),
			},
			{
				type: "toolCall",
				id: "call_a",
				name: "read",
				arguments: { path: "full/path", nested: { content: "unabridged" } },
				rawBlock: "<read full/path>",
				intent: "inspect",
	...
	const first: ToolResultMessage = {
		...
		toolName: "read",
		content: [
			{ type: "text", text: "result before image" },
			{ ...image },
			{ type: "text", text: "result after image" },
		],
		isError: true,
		details: { explanation: "readable failure metadata" },
		timestamp: 3,
	};
	const second: ToolResultMessage = {
		...
		toolCallId: "call_b",
		toolName: "edit",
		content: [{ type: "text", text: "complete patch result" }],
		isError: false,
		timestamp: 4,
	};
...
function model(baseUrl = "http://127.0.0.1:1/v1") {
	return buildModel({
		...
		provider: "openai",
		baseUrl,
		remoteCompaction: { v2StreamingEnabled: true },
		reasoning: true,
		input: ["text", "image"],
	...
	return input
		...
		.flatMap(block => {
			...
			try {
				...
				return value.type === "historical_context" ? [value] : [];
			} catch {
				return [];
			}
		});
...
test("V1 replaces a complete multi-tool atom with chronological logical history and real interleaved images", async () => {
	...
	expect(input.map(item => getSourceOrigin(item))).toEqual(
		expect.arrayContaining([
			expect.objectContaining({
				kind: "source",
				parts: expect.arrayContaining([
					expect.objectContaining({
						entryId: "local-source-1",
						blockIndex: 1,
						representation: "json-quoted-block",
						coverage: "full",
					}),
				]),
			}),
	...
	expect(
		input
			.map(item => getSourceOrigin(item))
			.flatMap(origin => (origin?.kind === "source" ? [origin.parts[0]!.entryId] : [])),
	).toEqual(data.sources.map(source => source.entryId));
	...
	expect(logical.find(value => value.block?.toolCallId === "call_a")?.block).toMatchObject({
		isError: true,
		details: { explanation: "readable failure metadata" },
	});
	...
	let sent: unknown;
	const canonical = [
		...input.map(item => ({ ...item, provider_extension: { untouched: true } })),
		{ type: "compaction", encrypted_content: "opaque-result", extra: "canonical" },
	];
	...
	try {
		...
		expect(new Set(result.allUserSources?.map(part => part.entryId))).toEqual(
			new Set(data.sources.map(source => source.entryId)),
		);
	} finally {
		server.stop(true);
	}
...
test("V2 failure rebuilds V1 historical N from frozen sources, not the attempted native payload", async () => {
	...
	const server = Bun.serve({
		...
		async fetch(request) {
			...
			canonical = [
				...body.input.filter(item => item.role === "user"),
				{ type: "compaction", encrypted_content: "canonical-fallback" },
			];
			return Response.json({ output: canonical });
		},
	});
	try {
		const preparation = {
			firstKeptEntryId: "local-source-4",
			messagesToSummarize: data.messages.slice(1, 4),
			turnPrefixMessages: [],
			recentMessages: [data.recent],
			isSplitTurn: false,
			tokensBefore: 100000,
			fileOps: createFileOps(),
			settings: { ...DEFAULT_COMPACTION_SETTINGS, remoteEnabled: true, remoteStreamingV2Enabled: true },
			sourcesToSummarize: data.sources.slice(1, 4),
			recentSources: data.sources.slice(4),
			selectedSources: data.sources.slice(0, 4),
		} as CompactionPreparation;
		const result = await compact(preparation, model(`${server.url}v1`), "offline-key", undefined, undefined, {
			remoteSystemPrompt: [requirementMarker],
		});
		...
		expect(
			requests[0]!.input.filter(item => item.type === "function_call" || item.type === "custom_tool_call"),
		).toHaveLength(2);
		expect(requests[1]!.input.every(item => item.role === "user")).toBe(true);
		expect(
			historicalPayloads(requests[1]!.input).find(value => value.block?.type === "thinking")?.block.thinking,
		).toBe("full plaintext thinking");
...
test("complete native atoms supplement plaintext and raw metadata without duplicating executable tool calls", () => {
	...
	expect(historicalPayloads(complete).find(value => value.block?.thinking)?.block.thinking).toBe(
		"full plaintext thinking",
	);
	expect(historicalPayloads(complete).find(value => value.block?.rawBlock === "<read full/path>")?.block.intent).toBe(
		"inspect",
	);
...
test("request trimming never replaces admitted N and marks ordinary rewritten output incomplete", () => {
	...
	expect(getSourceOrigin(ordinary.input.at(-1)!)).toMatchObject({
		kind: "source",
		parts: [
			expect.objectContaining({
				entryId: "local-source-3",
				coverage: "derived",
	...
	const protectedInput = trimRemoteCompactionInputToContextWindow(
		input,
		new Tokenizer(model()),
		2000,
		"compact",
	...
	expect(getSourceOrigin(protectedInput.input.at(-1)!)).toMatchObject({
		kind: "source",
		parts: [expect.objectContaining({ coverage: "full" })],
	});
});
			...
			const events = [
				{
					...
					item: { type: "compaction", encrypted_content: "same native summary" },
				},
	...
	try {
		const preparation: CompactionPreparation = {
			...
			settings: {
				...
				v2RetainedMessageBudget: 1,
			},
			sourcesToSummarize: data.sources.slice(1, 4),
			recentSources: data.sources.slice(4),
			selectedSources: data.sources.slice(1, 4),
		};
		const convertToLlm: ConvertToLlm = messages =>
			defaultConvertToLlm(messages).map(message =>
				message.role === "user" && typeof message.content === "string" && message.content.includes("keep")
					? transferTransformedSourceOrigin(message, {
							...message,
							content: message.content.replace("keep", "KEEP"),
						})
					: message,
			);
		const baseline = await compact(preparation, model(`${server.url}v1`), "offline-key", undefined, undefined, {
			convertToLlm,
			remoteSystemPrompt: [requirementMarker],
		});
		const selected = await compact(
			{
				...preparation,
				selectedSources: [
					{ ...data.sources[0]!, spans: [{ blockIndex: 0, start: 7, end: 11 }] },
					...data.sources.slice(1, 4),
				],
			},
			model(`${server.url}v1`),
			"offline-key",
			undefined,
			undefined,
			{ convertToLlm, remoteSystemPrompt: [requirementMarker] },
		);
		...
		expect(
			sent.filter(item => item.type === "function_call_output" || item.type === "custom_tool_call_output"),
		).toHaveLength(2);
		expect(historicalPayloads(sent).find(value => value.block?.thinking)?.block.thinking).toBe(
			"full plaintext thinking",
		);
		expect(historicalPayloads(sent).find(value => value.block?.rawBlock === "<read full/path>")?.block.intent).toBe(
			"inspect",
		);
...
test("historical selected computer results retain metadata-only screenshots and safety context", () => {
	const assistant: AssistantMessage = {
		...
		content: [
			{
				type: "toolCall",
				id: "call_screen|cu_screen",
				name: "computer",
				arguments: {},
				providerMetadata: {
					type: "computer",
					providerItemId: "cu_screen",
					actions: [{ type: "screenshot" }],
					pendingSafetyChecks: [{ id: "check", message: "pending safety" }],
				},
			},
		],
	};
	const result: ToolResultMessage = {
		role: "toolResult",
		toolCallId: "call_screen|cu_screen",
		toolName: "computer",
		content: [],
		isError: false,
		timestamp: 2,
		providerMetadata: {
			type: "computer",
			screenshot: { type: "computer_screenshot", image_url: `data:${image.mimeType};base64,${image.data}` },
			acknowledgedSafetyChecks: [{ id: "check", message: "acknowledged safety" }],
		},
	};
	...
	expect(getSourceOrigin(images[0])).toMatchObject({
		kind: "source",
		parts: [
			{
				entryId: "computer-result",
				blockIndex: "metadata.screenshot",
				coverage: "full",
	...
	expect(
		historicalPayloads(historical).find(value => value.block?.type === "toolCall")?.block.pendingSafetyChecks,
	).toEqual([{ id: "check", message: "pending safety" }]);
	expect(
		historicalPayloads(historical).find(value => value.block?.type === "toolResult")?.block.acknowledgedSafetyChecks,
	).toEqual([{ id: "check", message: "acknowledged safety" }]);
});
	...
	test(`compact V${streaming ? 2 : 1} preserves selected response-native server metadata without replaying an executable duplicate`, async () => {
		...
		const server = Bun.serve({
			...
			async fetch(request) {
				...
				const events = [
					{ type: "response.output_item.done", item: { type: "compaction", encrypted_content: "native-summary" } },
					{ type: "response.completed", response: { usage: { input_tokens: 30000, output_tokens: 1 } } },
				];
				return new Response(
		...
		try {
			const assistant: AssistantMessage = {
				...
				stopReason: "stop",
				timestamp: 1,
				content: [{ type: "text", text: "visible synthesis" }],
				providerPayload: {
					...
					items: [
						{
							...
							action: {
								type: "search",
								queries: ["selected native query"],
								sources: [{ type: "url", url: "https://example.invalid/selected-source" }],
							},
						},
					],
				},
			...
			const preparation: CompactionPreparation = {
				...
				settings: {
					...DEFAULT_COMPACTION_SETTINGS,
					remoteEnabled: true,
					remoteStreamingV2Enabled: streaming,
					v2RetainedMessageBudget: 1,
				},
				sourcesToSummarize: [selected],
				recentSources: [{ entryId: "recent-user", order: 1, message: recent }],
			...
			const search = streaming
				? executable[0]
				: logical.find(value => value.block?.type === "web_search_call")?.block;
...
test("selected historical native deltas retain interleaved image components but never admit full snapshots", () => {
	...
	expect(content[imageIndex]).toMatchObject({
		image_url: `data:${image.mimeType};base64,${image.data}`,
		detail: "original",
	});
	expect(JSON.parse(content[imageIndex + 1].text).block.text).toBe("native after");
	expect(getSourceOrigin(content[imageIndex])).toMatchObject({
		kind: "source",
		parts: [
			{
				entryId: "native-delta",
				blockIndex: "providerPayload.1.output.1",
				representation: "original-image",
				coverage: "full",
			},
		],
...
test("selected generated and code-interpreter images remain original image INPUT alongside native atoms", async () => {
		...
		providerPayload: {
			...
			items: [
				{ type: "image_generation_call", id: "ig_selected", status: "completed", result: jpeg },
				{
					...
					code: "render()",
					outputs: [
						{ type: "logs", logs: "before plot" },
						{ type: "image", url: `data:image/png;base64,${png}` },
						{ type: "logs", logs: "after plot" },
					],
				},
				{
					type: "reasoning",
	...
	expect(sourceTokenizer.countMessage(assistant, { excludeEncryptedReasoning: true })).toBe(
		2 * IMAGE_TOKEN_ESTIMATE +
			sourceTokenizer.countTokens(["render()", "before plot", "after plot", "readable reasoning"]),
	);
	...
	for (const input of [complete, historical]) {
		...
		expect(images.map(block => block.image_url)).toEqual([
		...
		expect(getSourceOrigin(images[0])).toMatchObject({
			...
			parts: [
				{
					entryId: "generated-images",
					blockIndex: "providerPayload.0.result",
					representation: "original-image",
					coverage: "full",
				},
			],
		});
		expect(getSourceOrigin(images[1])).toMatchObject({
			kind: "source",
			parts: [
				{
					entryId: "generated-images",
					blockIndex: "providerPayload.1.outputs.1",
					representation: "original-image",
					coverage: "full",
				},
			],
		...
		expect(
			content
				...
				.every(
					block =>
						!block.text.includes(jpeg) &&
						!block.text.includes(png) &&
						!block.text.includes("opaque-native-reasoning"),
				),
		).toBe(true);
	}
	expect(historicalPayloads(historical).find(value => value.block?.type === "reasoning")?.block.summary).toEqual([
		{ type: "summary_text", text: "readable reasoning" },
	]);
});
...
test("actual generated ingress and persisted replay quote each logical image once without byte deduplication", async () => {
	...
	const server = Bun.serve({
		...
		port: 0,
		fetch() {
			const events = [...generated, interpreter].map((item, output_index) => ({
				type: "response.output_item.done",
				item,
				output_index,
			}));
	...
	try {
		...
		const ingress = await streamOpenAIResponses(
			...
			{ apiKey: "offline-key", statefulResponses: false },
		).result();
		...
		expect(tokenizer.countMessage(replay, { excludeEncryptedReasoning: true })).toBe(
			3 * IMAGE_TOKEN_ESTIMATE + tokenizer.countTokens(["plot()", "before", "after"]),
		);
		const native = buildOpenAiNativeHistory([replay], active);
		for (const input of [
			buildCompleteNativeAtomItems(native, [replay]),
			composeOpenAiHistoricalInput(native, [], [replay]),
		]) {
			const images = input
				.flatMap(item => (Array.isArray(item.content) ? item.content : []))
				.filter(block => block.type === "input_image");
			expect(images.map(block => block.image_url)).toEqual(
				Array(3).fill(`data:${image.mimeType};base64,${image.data}`),
			);
		}
