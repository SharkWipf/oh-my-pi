...
describe("compaction source continuity", () => {
	test("replacement bytes are missing current coverage while historical pixels keep their source positions", () => {
		...
		const preserve = {
			sourceRepresentation: captured,
			snapcompact: { text: "ABCDEF", frames: [{ data: "unchanged" }] },
		};
		...
		const surviving = mapped.coverage.flatMap(run =>
			run.current ? [source.slice(run.current.start, run.current.end)] : [],
		);
	...
	test("insertion and subsequent edit follow current coordinates without crediting inserted content", () => {
		const inserted = remapCompactionSourceRepresentation({ sourceRepresentation: capturedSource() }, [
			{
				entryId: "user",
				blocks: [{ oldBlockIndex: 1, newBlockIndex: 1, textEdits: [{ start: 2, end: 2, replacementLength: 3 }] }],
			},
		]);
		const replaced = remapCompactionSourceRepresentation(inserted, [
			{
				entryId: "user",
				blocks: [{ oldBlockIndex: 1, newBlockIndex: 1, textEdits: [{ start: 6, end: 7, replacementLength: 1 }] }],
			},
		]);
		const mapped = getCompactionSourceRepresentation(replaced)!;
		const currentSource = "ABxyzCQEF";
		expect(
			mapped.coverage.flatMap(run => (run.current ? [currentSource.slice(run.current.start, run.current.end)] : [])),
		).toEqual(["AB", "C", "EF"]);
		expect(mapped.coverage.filter(run => run.status === "historical-not-current").map(run => run.snapshot)).toEqual([
			{ blockIndex: 1, start: 3, end: 4 },
		]);
	});
	...
	test("an unavailable positional map cannot claim exact current coverage or alter another source ID", () => {
		...
		const result = remapCompactionSourceRepresentation(
			{
				sourceRepresentation: {
					...one,
					coverage: [...one.coverage, ...two.coverage],
					layout: [...one.layout, ...two.layout],
				},
			},
			[{ entryId: "one" }],
		);
