...
- Restored image-only drafts now expose deletable attachment chips while resubmitting the exact original text and image links, including `/keep` and `/once`; rejected submissions retain the same editable projection.
- A failed context-reset preflight remains visible, and a new explicit same-scope reset can recover through the guarded session journal without overwriting a concurrent writer.
- Restored an always-visible `/context` legend distinguishing saved manual overrides from current compaction-selection markers; Inspect now explains automatic `A` selection without implying a checkbox change or already-installed content.
...
- Live requirements recall now composes with staged ordinary-memory delivery and the last uncomposed request policy, including synthetic continuations and failed-switch rollback; ordinary Hindsight boundary and manual reload behavior remain independent.
