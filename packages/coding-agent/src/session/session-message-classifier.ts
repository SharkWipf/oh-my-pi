...
export type MessageClassificationListener = (
...
export class SessionMessageClassifier {
	...
	constructor(host: SessionMessageClassifierHost) {
		this.#host = host;
	}
	...
	#notify(ids: readonly string[] = []): void {
		...
		for (const listener of this.#listeners) {
			try {
				...
				logger.error("Classifier status listener failed", { error: String(error) });
			}
		}
	}
	#resolveModel(): Model {
	...
	async getAvailability(): Promise<MessageClassificationAvailability> {
		try {
			...
			return { available: false, reason: String(error instanceof Error ? error.message : error) };
		}
	}
	async #scope(targetId?: string): Promise<ClassifierScope> {
		const manager = this.#host.sessionManager;
		const scope: ClassifierScope = {
			entries: [],
			boundaryId: null,
			anchorId: manager.getLeafId(),
		...
		while (cursor !== null) {
			if (performance.now() >= deadline) {
				...
				if (
					...
					scope.generation !== this.#host.generation()
				)
					throw new Error("Classifier source scan interrupted.");
		...
		if (targetId !== undefined && scope.entries.length === 0)
			throw new Error("Classifier target is not in the active post-clear branch.");
	...
	*#currentEntries(entries: readonly SessionEntry[]): Iterable<SessionEntry> {
	...
	#current(job: Job): boolean {
		if (
			...
			job.generation !== this.#host.generation() ||
			job.status.sessionId !== this.#host.sessionManager.getSessionId()
		)
			return false;
	...
	async #ready(job: Job): Promise<boolean> {
		...
		this.#cancel(
			job,
			"interrupted",
			"Session, branch, reset, or source scope changed; resume missing classifications explicitly.",
		);
		return false;
	}
	...
	start(entryId: string, kind: "selected" | "live" = "selected"): Promise<string> {
		...
		const start = (async () => {
			...
			const isCurrent = () =>
				this.#starts.get(entryId) === start &&
				scope.epoch === this.#epoch &&
				scope.sessionId === this.#host.sessionManager.getSessionId() &&
				scope.generation === this.#host.generation();
			for await (const target of iteratePreservedUserMessageClassifierInputsCooperatively(
				this.#currentEntries(scope.entries),
				isCurrent,
			)) {
		...
		void start.catch(() => {
			if (this.#starts.get(entryId) === start) this.#starts.delete(entryId);
		});
		return start;
	}
	async startBackfill(workers: number): Promise<string> {
		if (!Number.isSafeInteger(workers) || workers <= 0)
			throw new Error("Classifier workers must be a positive integer.");
		const scope = await this.#scope();
		const host = this.#host;
		const currentEntries = () => this.#currentEntries(scope.entries);
		async function* missing(
			isCurrent: () => boolean,
			isInvalidated: (entryId: string) => boolean,
		): AsyncGenerator<ClassifierTarget> {
	...
	async #start(
		...
		createIterator: (isCurrent: () => boolean, isInvalidated: (entryId: string) => boolean) => Job["iterator"],
		targetId?: string,
	): Promise<string> {
		...
		const job: Job = {
			...
			validationLeafId: scope.anchorId,
			epoch: scope.epoch,
			status: {
				id: Bun.randomUUIDv7(),
				kind,
	...
	async #launch(job: Job): Promise<void> {
		try {
			...
			if (job.status.state === "running") {
				...
				job.status.error = String(error instanceof Error ? error.message : error);
			}
		} finally {
	...
	async #next(job: Job): Promise<Work | undefined> {
		...
		for (;;) {
			...
			if (!this.#current(job)) {
				this.#cancel(job, "interrupted", "Classifier source scope changed before queueing.");
				return undefined;
			}
			if (next.done) {
				job.finishedScanning = true;
				return undefined;
			}
			if (job.invalidatedTargets.delete(next.value.entryId)) {
				job.status.failed++;
				job.status.completed++;
				this.#rows.set(next.value.entryId, {
					entryId: next.value.entryId,
					jobId: job.status.id,
					state: "interrupted",
					error: "Source/context changed; explicit retry required.",
				});
			...
			if (shared) {
				job.status.queued++;
				const observed = untilAborted(job.controller.signal, shared.completion.promise)
					.then(saved => {
						if (!this.#current(job)) return;
						if (saved && !job.invalidatedTargets.has(shared.entryId) && this.#inputMatches(shared.input))
							job.status.saved++;
						else job.status.failed++;
					})
					.catch(() => {
						if (!job.controller.signal.aborted) job.status.failed++;
					})
					.finally(() => {
	...
	async #run(work: Work): Promise<void> {
		...
		try {
			...
			if (!this.#inputMatches(input))
				throw new Error("Classifier source/context changed; retry with current input.");
					...
					const options = this.#host.prepareOptions(
						{
							...
							promptCacheKey: `${job.status.sessionId}:message-classifier`,
			...
			this.#host.sessionManager.appendCustomEntry(
				USER_MESSAGE_CLASSIFICATION_CUSTOM_TYPE,
				packPreservedUserMessageClassifications([{ id: entryId, mask }]),
			);
			await this.#host.sessionManager.flush();
			if (!(await this.#ready(job))) return;
			signal.throwIfAborted();
			if (!this.#current(job) || this.#paused || !this.#inputMatches(input)) {
	...
	cancel(id: string): void {
		...
		if (job)
			this.#cancel(
				job,
				"canceled",
				"Canceled; saved facts remain active. Resume missing classifications explicitly.",
			);
	}
	...
	resume(): void {
		...
		for (const job of this.#jobs.values())
			if (!this.#current(job))
				this.#cancel(job, "interrupted", "Session or branch changed; resume missing classifications explicitly.");
		this.drainLive();
	}
	...
	interruptInputs(ids: readonly string[]): void {
		...
		for (const id of ids) {
			...
			const row = this.#rows.get(id)!;
			row.state = "interrupted";
			row.error = "Source/context changed; explicit retry required.";
			work.completion.resolve(false);
			this.#pending.delete(id);
		}
		this.#notify(ids);
	...
	dispose(): void {
		this.interrupt("Session disposed; restart requires explicit missing-only backfill.");
		this.#paused?.resolve();
		this.#paused = undefined;
	...
	enqueueLive(entryId: string): void {
		if (
			!this.#host.settings.get("compaction.keepUserMessages") ||
			!this.#host.settings.get("compaction.keepUserMessagesLlm")
		)
			return;
	...
	drainLive(): void {
		...
		void this.start(entryId, "live")
			.then(jobId => {
				if (this.#jobs.get(jobId)?.status.kind !== "live") {
					this.#liveActive = false;
					this.drainLive();
				}
			})
