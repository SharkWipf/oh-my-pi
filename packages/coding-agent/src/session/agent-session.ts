...
export class AgentSession {
	...
	async #queueUserMessage(
		...
		if (mode === "aside") {
			...
			records.push({ role: "user", content, attribution, timestamp: timestamp ?? Date.now(), producer });
			this.#irc.queueAside(records);
	...
	async sendUserMessage(
		...
		if (options?.deliverAs === "aside") {
			if (this.isStreaming) {
				await this.#queueUserMessage(text, images, "aside", { attribution: options.attribution, producer: options.producer });
				return;
			}
			...
			await this.#queueUserMessage(text, images, "followUp", { attribution: options.attribution, producer: options.producer });
			return;
		} else if (options?.deliverAs === "steer") {
			await this.#queueUserMessage(text, images, "steer", { attribution: options.attribution, producer: options.producer });
			return;
		}
