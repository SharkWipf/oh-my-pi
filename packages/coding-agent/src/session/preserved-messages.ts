...
export function evaluatePreservationPolicy(
	...
	const heuristic = policy.heuristics && !hasNonText && (!text.trim() || PURE_ACKNOWLEDGMENT.test(text.trim()))
		? "exclude" : "auto";
	...
	if (policy.classifier && categoryMask !== undefined) {
		for (let bit = 0; bit < PRESERVED_USER_MESSAGE_CATEGORIES.length; bit++) {
			if (categoryMask & (1 << bit)) classifier = strongest(classifier, policy.categoryActions[PRESERVED_USER_MESSAGE_CATEGORIES[bit]!]);
		}
	}
...
function fullSpans(id: string, message: AgentMessage): PreservationSourceSpan[] {
	...
	if (typeof message.content === "string") return [{ sourceId: id, blockIndex: 0, text: { start: 0, end: message.content.length } }];
	if (!Array.isArray(message.content)) return [];
	return message.content.map((block, blockIndex) => ({ sourceId: id, blockIndex,
...
function safeSuffix(text: string, start: number): number {
	if (start > 0 && start < text.length && /[\uD800-\uDBFF]/.test(text[start - 1]!) && /[\uDC00-\uDFFF]/.test(text[start]!)) return start + 1;
	return start;
}

/** Prune text only, retaining original non-text blocks at their authored positions. */
function pruneAt(entry: SessionMessageEntry & { message: UserMessage }, head: number, tail: number): Pick<PreservationCandidate, "message" | "spans"> {
	const source = entry.message;
	const blocks = typeof source.content === "string" ? [{ type: "text" as const, text: source.content }] : source.content;
	let length = 0;
	...
	for (let blockIndex = 0; blockIndex < blocks.length; blockIndex++) {
		...
		if (end === block.text.length || start === 0) {
			...
			if (!marked && start > end) { text += "[truncated]"; marked = true; }
			text += block.text.slice(start);
			if (start < block.text.length) spans.push({ sourceId: entry.id, blockIndex, text: { start, end: block.text.length } });
			if (text) content.push({ ...block, text });
		}
		offset += block.text.length;
	}
	return { message: { ...source, content: typeof source.content === "string" ? content.filter(block => block.type === "text").map(block => block.text).join("") : content, providerPayload: undefined }, spans };
}
...
export function preservationCandidate(
	...
	const at = (retained: number) => pruneAt(source,
	...
	if (tokens > policy.maxTokens) return { ...base, ...chosen, quotaTokens: tokens, truncated: length > 0,
	...
	while (low < high) {
		...
		if (price <= policy.maxTokens) { low = middle; chosen = candidate; tokens = price; }
		else high = middle - 1;
	}
...
export function prechargeNonUsers(
	...
	let tokens = 0;
	for (const atom of atoms) for (const entry of atom.entries) {
...
interface PendingExchange { members: string[]; remaining: number }
...
export class PreservedMessageQuery {
	...
	static async build(entries: readonly SessionEntry[], policy: PreservationPolicySettings, tokenizer: Tokenizer, options: PreservationBuildOptions): Promise<PreservedMessageQuery | undefined> {
		...
		while (offset > 0 && entries[offset - 1]!.type !== "reset_boundary") {
			offset--;
			if (performance.now() >= deadline) { await cooperate(); if (!options.isCurrent()) return undefined; }
		}
	...
	#assertCurrent(): void {
		if (!this.#published || !this.#ownership.isCurrent()) throw new Error("Preservation source branch is no longer current");
	}
		...
		if (entry.message.role === "assistant") {
			...
			if (exchange) {
				...
				if (--exchange.remaining === 0) {
					...
					while (low < high) { const middle = (low + high) >>> 1; if (this.#rows[middle]! < anchor) low = middle + 1; else high = middle; }
	...
	#refreshPosition(position: number): void {
		...
		if (isPreservationUser(entry)) {
			...
			if (manual === "keep") {
				...
				for (const id of members) raw += this.#tokenizer.countMessage(this.#entry(this.#positions.get(id)!)!.message);
			} else this.#manualAtoms.delete(position);
			this.#index.update(position, { user: false, raw, candidate: 0, eligible: false, always: manual === "keep", manual: false, nonUserCount: manual === "keep" ? members.length : 0 });
		}
	}

	rowCount(role: "user" | "all" = "user"): number { this.#assertCurrent(); return (role === "user" ? this.#users : this.#rows).length; }
	...
	rowIndexOf(id: string, role: "user" | "all" = "user"): number {
		...
		while (low < high) { const middle = (low + high) >>> 1; if (rows[middle]! < position) low = middle + 1; else high = middle; }
	...
	inspect(id: string): PreservationStages | undefined {
		...
		return isPreservationUser(entry) ? evaluatePreservationPolicy(entry.message, this.#policy, manual, this.#classifications.get(id)) :
	...
	applyManualOverride(ids: readonly string[], state: PreservationAction): readonly string[] {
		...
		for (const id of ids) if (this.#positions.has(id)) for (const member of this.#groups.get(id) ?? [id]) affected.add(member);
		for (const id of affected) { if (state === "auto") this.#overrides.delete(id); else this.#overrides.set(id, state); }
	...
	select(options: PreservationSelectionOptions): PreservationSelection {
		...
		const P = membership(totalP.count, id => { owner.#assertCurrent(); const position = owner.#positions.get(id); return position !== undefined && selectedUser(position); }, function* () {
			...
			for (;;) {
				...
				for (let index = 0; index < heads.length; index++) if (!heads[index]!.done && heads[index]!.value === next) heads[index] = iterators[index]!.next();
			}
		});
		...
		return { P, N, H: hard, quota: { P: totalP, N: totalN, H, first, recent, always },
			...
			*candidates() {
				...
				while (!u.done || !n.done) {
					if (!u.done && (n.done || owner.#positions.get(u.value)! < owner.#positions.get(n.value)!)) { yield candidate(u.value)!; u = users.next(); }
					else if (!n.done) { yield candidate(n.value)!; n = others.next(); }
				}
			},
			*nonUserAtoms() {
				owner.#assertCurrent();
