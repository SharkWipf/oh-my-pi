import { remapCompactionSourceRepresentation, type SourceRewrite } from "@oh-my-pi/pi-agent-core/compaction/source";
...
function collectTargets(entryId: string, getEntry: EntryLookup, childrenOf: ChildLookup, targets: Set<string>): void {
...
export function rewriteSessionSources(
	...
	for (const rewrite of rewrites) {
		...
		if (
			...
			entry.message.providerPayload.contentBlocks?.length
		)
			bindNativeIngress = true;
	...
	for (const [id, originalEntry] of originals) {
		...
		if (
			entry?.type !== "message" ||
			originalEntry.type !== "message" ||
			("requirementsInvalidated" in entry && entry.requirementsInvalidated)
		)
			continue;
		...
		if (
			...
			(original.role !== "user" && original.role !== "custom") ||
			message.originalSubmission
		)
			continue;
	...
	for (const entry of entries) {
		if (entry.type === "compaction") {
			...
			if (native && typeof native === "object") {
				const replacementOrigins =
					"replacementOrigins" in native && Array.isArray(native.replacementOrigins)
						? remapNativeItemOrigins(native.replacementOrigins, rewrites)
						: undefined;
				const allUserOrigin =
					"allUserSources" in native && Array.isArray(native.allUserSources)
						? remapNativeItemOrigins([{ kind: "source", parts: native.allUserSources }], rewrites)[0]
						: undefined;
		...
		if (
			affected.size === 0 ||
			entry.type !== "custom" ||
			entry.customType !== USER_MESSAGE_CLASSIFICATION_CUSTOM_TYPE
		) {
			continue;
		}
