...
export class SessionMaintenance {
	...
	async #commitCompactionEntry(args: CompactionCommit): Promise<CompactionEntry | undefined> {
		...
		const tokensAfter = isRecord(frozen.details) && frozen.details.kind === "experimental-context-rollover" ? this.#projectExperimentalContextRolloverTokens(frozen) : this.#projectCompactedContextTokens(frozen);
