...
import { type BlobPutOptions, type BlobPutResult, BlobStore, isBlobRef, parseBlobRef, lazyImageDataSync } from "./blob-store";
...
export class SessionManager {
	...
	buildSessionContext(options?: BuildSessionContextOptions): SessionContext {
		...
		if (contextOptions.transcript && !contextOptions.collapseCompactedHistory) {
			return buildSessionContext(this.#entries, this.#index.leafId(), this.#index.entriesById(), contextOptions);
		}
		const { path, controls } = this.#index.contextPath(contextOptions);
		return buildSessionContextFromPath(path, contextOptions, controls);
	}
