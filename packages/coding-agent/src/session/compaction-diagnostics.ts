...
function quantity(
	tokens: number | null,
	description: string,
	basis: DiagnosticTokenQuantity["basis"] = "local-estimate",
): DiagnosticTokenQuantity {
...
export function describeCompactionSourceRepresentation(
	representation: SourceRepresentation | undefined,
	id: string,
): string[] {
	...
	for (const run of sourceRuns) {
		...
		const current = run.current
			? `current block ${run.current.blockIndex} [${run.current.start}, ${run.current.end})`
			: "current correspondence unavailable";
	...
	for (let index = 0; index < representation.layout.length; index++) {
		...
		if (part.kind === "source" && part.entryId === id) {
			...
			descriptions.push(
				`Occurrence ${index}: original image captured at block ${part.blockIndex}; ${part.currentBlockIndex === undefined ? "current reference unresolved" : `current block ${part.currentBlockIndex}`}.`,
			);
		} else if (part.kind === "text" || part.kind === "frame") {
			const covered = sourceRuns.some(
				run => run.normalized && run.normalized.start < part.range.end && run.normalized.end > part.range.start,
			);
			if (covered)
				descriptions.push(
					`Occurrence ${index}: ${part.kind === "frame" ? `frame ${part.frameIndex}` : "archive text"}, archive range [${part.range.start}, ${part.range.end}); may share physical content with other sources, no per-source token allocation.`,
				);
		}
	}
	if (representation.aggregate?.entryIds.includes(id))
		descriptions.push(
			`${representation.aggregate.reason} aggregate reference only; not exact retained source position or coverage.`,
		);
	return descriptions.length > 0
		? descriptions
		: [
				"No coverage for this source is recorded in this representation; ordinary/later context membership is separate.",
			];
}

function sourceCoverage(
	part: SourceLayoutPart | undefined,
	representation: SourceRepresentation | undefined,
): Pick<ContextInventoryRow, "coverage" | "sourceIds" | "contributions" | "note"> {
	if (!part || !representation) return { coverage: "unknown" };
	if (part.kind === "gap") return { coverage: "aggregate", note: part.reason };
	if (part.kind === "source" && part.contribution)
		return { coverage: "source", sourceIds: [part.entryId], contributions: [part.contribution] };
					...
					(part.kind === "original-image"
						...
						: !part.spans ||
							part.spans.some(
								span =>
									run.current &&
									span.blockIndex === run.current.blockIndex &&
									span.start < run.current.end &&
									span.end > run.current.start,
							));
...
function preparedOriginCoverage(
	origin: NativeItemOrigin | undefined,
	input: CompactionDiagnosticsInput,
	rawPart?: Extract<SourceLayoutPart, { kind: "source" }>,
): Pick<ContextInventoryRow, "coverage" | "sourceIds" | "contributions"> {
	...
	for (const part of origin.parts) {
		...
		if (part.coverage === "derived" || part.status === "unknown") {
			missingContribution = true;
			continue;
		}
		if (
			(!part.projection && input.postCompactionSourceIds?.has(part.entryId)) ||
			(input.method === "uncompacted" && !input.sourceRepresentation)
		) {
			contributions.add("ordinary");
			continue;
		}
		const range =
			part.sourceSpan ??
			(part.sourceLength !== undefined
				? { start: 0, end: part.sourceLength }
				: part.representation === "original-image"
					? { start: 0, end: 1 }
					: undefined);
		const currentRange =
			part.currentSourceSpan ?? (part.status === undefined || part.status === "exact-current" ? range : undefined);
		const currentBlock = part.currentBlockIndex ?? part.blockIndex;
		let wholePart = false;
		if (!rawPart && input.sourceRepresentation)
			for (const layout of input.sourceRepresentation.layout) {
				if (
					layout.kind !== "source" ||
					layout.entryId !== part.entryId ||
					layout.projection !== part.projection ||
					!layout.contribution ||
				...
				if (
					...
					(!currentRange ||
						!layout.spans.some(
							span =>
								span.blockIndex === currentBlock &&
								span.start <= currentRange.start &&
								span.end >= currentRange.end,
						))
				)
					continue;
		...
		for (const run of input.sourceRepresentation?.coverage ?? []) {
			if (
				...
				run.projection !== part.projection ||
				run.snapshot.blockIndex !== part.blockIndex ||
				run.snapshot.start >= range.end ||
				run.snapshot.end <= range.start
			)
				continue;
	...
	return {
		coverage: unknown || ids.size === 0 ? "unknown" : historical ? "historical" : "source",
		sourceIds: [...ids],
		contributions: missingContribution ? [] : [...contributions],
	};
}

function appendNativeRows(
	...
	for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
		...
		let blockIndex = 0;
		const emit = (
			kind: ContextInventoryRow["kind"],
			label: string,
			tokens: number | null,
			size: ContextInventoryRow["payloadSize"],
			blocks = 1,
		) => {
			rows.push({
				...
				quantity: quantity(
					tokens,
					tokens === null
						? "Opaque native item cost is unknown"
						: kind === "original-image"
							? "Native image baseline estimate, not a provider invoice"
							: "Visible native content under the local tokenizer estimator",
					kind === "original-image" ? "image-estimate" : "local-estimate",
				),
				counts: {
					messages:
						rows.length === firstRow &&
						(item.type === "message" || (item.type === undefined && typeof item.role === "string"))
							? 1
							: 0,
					items: rows.length === firstRow ? 1 : 0,
					blocks,
					frames: 0,
					images: kind === "original-image" ? 1 : 0,
				},
				...(size ? { payloadSize: size } : {}),
				controls: ["native compaction", "model"],
				note: "Native returned content has no exact source-position attribution.",
			});
			if (tokens !== null) known += tokens;
		};
		const text = (value: string, label: string, kind: "text" | "tool" = "text") =>
			emit(kind, label, tokenizer.countTokens(value), {
				value: Buffer.byteLength(value, "utf8"),
				unit: "utf8-bytes",
			});
		const opaque = (value: unknown, label: string, blocks = 1) =>
			emit(
				"native",
				label,
				null,
				{ value: Buffer.byteLength(JSON.stringify(value) ?? "", "utf8"), unit: "utf8-bytes" },
				blocks,
			);
		const unresolved = (value: unknown, label: string, blocks = 1) => {
			const start = rows.length;
			if (value && typeof value === "object")
				visitOpenAIResponsesLogicalContent(value, {
					image: image => {
						const url = image.type === "reference" ? image.image_url : undefined;
						const prefix = url?.startsWith("data:") ? url.indexOf(";base64,") : -1;
						const size: ContextInventoryRow["payloadSize"] =
							image.type === "base64"
			...
			if (rows.length === start) opaque(value, label, blocks);
				...
				!(
					...
					typeof value === "object" &&
					"type" in value &&
					(value.type === "input_image" || value.type === "computer_screenshot" || value.type === "image")
				)
			) {
		...
		const content = (value: unknown) => {
			if (typeof value === "string") {
				text(value, "Native message text");
				return;
			}
			if (!Array.isArray(value)) {
				unresolved(value, "Unresolved native content");
				return;
			}
			for (const block of value) {
				if (!block || typeof block !== "object") {
					opaque(block, "Unresolved native block");
					continue;
				}
				if (
					(block.type === "input_text" || block.type === "output_text" || block.type === "summary_text") &&
		...
		if ((item.type === "message" || item.type === undefined) && typeof item.role === "string") {
			...
			emit("tool", "Native function call", tokenizer.countTokens([item.name, item.arguments]), {
				value: Buffer.byteLength(item.name, "utf8") + Buffer.byteLength(item.arguments, "utf8"),
				unit: "utf8-bytes",
			});
...
function coalesceAdjacentRows(rows: ContextInventoryRow[]): void {
	let length = 0;
	const equal = (a: readonly string[] | undefined, b: readonly string[] | undefined) =>
		a === b || (!!a && !!b && a.length === b.length && a.every(value => b.includes(value)));
	for (const row of rows) {
		const previous = rows[length - 1];
		const mergeable =
			row.kind === "text" || row.kind === "tool" || row.kind === "retained" || row.kind === "post-compaction";
		if (
			mergeable &&
			previous &&
			previous.kind === row.kind &&
			previous.label === row.label &&
			previous.coverage === row.coverage &&
			previous.note === row.note &&
			previous.quantity.tokens !== null &&
			row.quantity.tokens !== null &&
			previous.quantity.basis === row.quantity.basis &&
			previous.payloadSize?.unit === row.payloadSize?.unit &&
			equal(previous.selectionReasons, row.selectionReasons) &&
			equal(previous.contributions, row.contributions) &&
			equal(previous.controls, row.controls)
		) {
			const delimiter = previous.location.indexOf(" .. ");
			previous.location =
				(delimiter < 0 ? previous.location : previous.location.slice(0, delimiter)) + " .. " + row.location;
...
export function buildCompactionDiagnostics(input: CompactionDiagnosticsInput): CompactionDiagnostics {
	...
	const fixedCosts = prepared
		? {
				...
				systemContextTokens: input.tokenizer.countTokens(prompt.slice(1)),
				skillsTokens: 0,
			}
		: input.fixedCosts;
	const fixedCounts = prepared
		? {
				systemPrompt: Math.min(1, prompt.length),
				tools: prepared.tools?.length ?? 0,
				context: Math.max(0, prompt.length - 1),
				skills: 0,
			}
		: input.fixedCounts;
	...
	for (const location of input.sourceLocations ?? []) {
		...
		const blocks =
			source.role === "compactionSummary"
				? source.blocks
				: "content" in source && Array.isArray(source.content)
					? source.content
					: undefined;
	...
	const pushFixed = (
		kind: "system-prompt" | "tools" | "context" | "skills",
		tokens: number,
		label: string,
		...
		rows.push({
			...
			counts: { messages: 0, blocks: 0, frames: 0, images: 0, ...(items === undefined ? {} : { items }) },
			controls: ["model", kind],
	...
	for (let messageIndex = 0; messageIndex < inventoryMessages.length; messageIndex++) {
		...
		const converted = prepared
			? [prepared.messages[messageIndex]!]
			: convertToLlm(original.role === "assistant" && next ? [original, next] : [original]);
		const fragmentCount =
			!prepared && original.role === "assistant" && next
				? converted.length - convertToLlm([next]).length
				: converted.length;
		...
		for (let fragmentIndex = 0; fragmentIndex < fragments.length; fragmentIndex++) {
			...
			const emittedBlock = (index: number | undefined): object | undefined =>
				index !== undefined && Array.isArray(message.content) ? message.content[index] : undefined;
			...
			const add = (
				...
				const coverage = physical
					? {
							coverage: physical.owner === "tool" ? ("aggregate" as const) : ("fixed" as const),
							contributions: ["ordinary" as const],
						}
					: kind === "native" || kind === "summary"
						? { coverage: "aggregate" as const }
						: part?.kind === "source" && !part.contribution && origin?.kind === "source"
							? preparedOriginCoverage(origin, input, part)
							: part
								? sourceCoverage(part, input.sourceRepresentation)
								: prepared
									? preparedOriginCoverage(origin, input)
									: sourceId
										? { coverage: "source" as const, sourceIds: [sourceId] }
										: { coverage: "unknown" as const };
				const reasons = new Set<string>();
				for (const id of coverage.sourceIds ?? [])
					for (const reason of input.selectionReasons?.get(id) ?? []) reasons.add(reason);
				const actualKind =
					physical?.kind === "note"
						? physical.owner === "system"
							? "system-prompt"
							: physical.owner === "context"
								? "context"
								: "tool"
						: part?.kind === "gap"
							? "gap"
							: kind === "text" && sourceId && !part
								? input.postCompactionSourceIds?.has(sourceId)
									? "post-compaction"
									: "retained"
								: kind;
				rows.push({
					...
					quantity: quantity(
						tokens,
						tokens === null
							? "Provider replay cost is not locally measurable"
							: physical?.kind === "frame"
								? "Live inline renderer image estimate, not a provider invoice"
								: image
									? "Generic local image/frame estimate, not a provider invoice"
									: "Emitted content under the local tokenizer estimator",
						image ? "image-estimate" : "local-estimate",
					),
					counts: {
						messages: rows.length === rowStart ? 1 : 0,
						blocks: 1,
						frames: kind === "frame" ? 1 : 0,
						images: image ? 1 : 0,
					},
					...(bytes === undefined
						? {}
						: {
								payloadSize: {
									value: bytes,
									unit: image ? ("base64-characters" as const) : ("utf8-bytes" as const),
								},
							}),
					controls: physical
						? [physical.owner === "tool" ? "snapcompact.toolResults" : "snapcompact.systemPrompt", "model"]
						: image
							? kind === "frame"
								? ["snapcompact.shape", "archive frame window", "archive payload budget", "model"]
								: ["images", "model"]
							: ["compaction", "model"],
					...(note ? { note } : {}),
				});
			...
			if (payload?.type === "anthropicCompaction" && payload.provider === input.model.provider) {
				...
				add(
					"summary",
					"Anthropic native compaction summary",
					summaryTokens,
					undefined,
					Buffer.byteLength(payload.content, "utf8"),
					false,
					"Provider-native summary is aggregate content, not exact retained source. Local visible-text estimate only.",
				);
				if (payload.encryptedContent) {
					add(
						"native",
						"Anthropic encrypted compaction state",
						null,
						undefined,
						Buffer.byteLength(payload.encryptedContent, "utf8"),
						false,
						"Opaque replay content",
					);
				...
				if (replacesFallback && payload.filesText?.trim()) {
					...
					add(
						"summary",
						"Anthropic compaction file metadata",
						filesTokens,
						undefined,
						Buffer.byteLength(payload.filesText, "utf8"),
						false,
						"Native payload file metadata, without exact source attribution. Final provider placement may follow retained assistant/tool messages; this inventory follows OMP Context order, not provider wire order.",
					);
			...
			if (
				...
				(payload.provider === undefined || payload.provider === input.model.provider)
			) {
				...
				for (let index = nativeStart; index < rows.length; index++)
					hasUnknown ||= rows[index]!.quantity.tokens === null;
				continue;
			}
			const content =
			...
			for (let blockIndex = 0; blockIndex < content.length; blockIndex++) {
				...
				if (block.type === "image") {
					...
					const frame =
						physical?.kind === "frame" ||
						archiveFrame !== undefined ||
						part?.kind === "frame" ||
						(original.role === "compactionSummary" && part?.kind !== "original-image" && !originalImage);
					const tokens =
						physical?.kind === "frame" ? physical.estimatedTokens : frame ? framePrice : IMAGE_TOKEN_ESTIMATE;
					imageAdjustment += tokens - fullTokens;
					add(
						frame ? "frame" : "original-image",
						physical?.kind === "frame"
							? `Inline ${physical.owner} frame`
							: frame
								? "Rasterized archive frame"
								: "Original or unclassified image",
						tokens,
						blockIndex,
						block.data.length,
						true,
						archiveFrame
							? "Archive identity is recorded; historical renderer-specific price is not. Charge is the generic local archive estimate."
							: undefined,
					);
					continue;
				}
						...
						block.type === "text"
							...
							: block.type === "thinking"
								...
								: block.type === "toolCall"
									...
									: "";
					const kind =
						(original.role === "compactionSummary" && blockIndex === 0) || original.role === "branchSummary"
							? "summary"
							: block.type === "toolCall" || message.role === "toolResult"
								? "tool"
								: "text";
					add(
						kind,
						kind === "summary" ? "Compaction summary and wrapper" : `${message.role} ${block.type}`,
						visibleTokens,
						blockIndex,
	...
	const fixedTotal =
		fixedCosts.systemPromptTokens + fixedCosts.toolsTokens + fixedCosts.systemContextTokens + fixedCosts.skillsTokens;
	const reconciled =
		fixedTotal +
		convertedEstimate +
		imageAdjustment -
		excludedOpaqueEstimate -
		nativePlaceholderEstimate +
		nativeKnownEstimate;
	const notes = [
		prepared
			? "Actual supplied OMP-prehook Context: transformed prompt, descriptors and message blocks only. Inline frame prices come from live renderer facts; no hooks or rendering rerun. Skills remain included in transformed prompt text without separate attribution."
			: "Local emitted-content estimate after message conversion; includes summary wrappers, excludes unmeasurable opaque replay. Not a provider invoice or a post-hook observation.",
	...
	if (input.frameTokenEstimate !== undefined)
		notes.push(
			`Separate shape-family estimate: ${input.frameTokenEstimate} tokens/frame; not substituted for the ${FRAME_TOKEN_ESTIMATE} generic archive charge in this local domain.`,
		);
	if (!input.selectedUserSourceIds)
		notes.push(
			"Selected-user source count contains known contributed additions only; overlapping ordinary policy membership may be unavailable.",
		);
	if (unallocated)
		notes.push(
			"Shared/unallocated includes emitted content without exact source attribution; no exact per-selection allocation is claimed.",
		);
	if (reconciled !== total)
		notes.push(
			`Row subtotal ${total} differs from aggregate estimator ${reconciled}; tokenizer fragmentation is not an exact provider serialization count.`,
		);
	return {
		snapshot: input.snapshot ?? (prepared ? "OMP-prehook" : "recorded-at-compaction"),
		...
		total: quantity(
			total,
			hasUnknown
				? "Known local subtotal; opaque/native replay cost remains unknown"
				: "Sum of disjoint emitted-content local estimates",
		),
		rows,
		distribution: {
		...
		...(hasUnknown
			? {
					warning:
						"Opaque/native replay has unknown local token cost; the known subtotal is not the complete request size.",
				}
			: {}),
		notes,
	};
