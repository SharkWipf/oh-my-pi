...
export function buildPreservedUserMessageClassifierRequest(
	...
	if (model.contextWindow !== null && model.contextWindow <= 0) {
		throw new Error(
			`User-message classifier: ${modelName} has an invalid input allowance. Configure a supported model.`,
		);
	}
	...
	const fits = (context: Context): boolean => {
		if (allowance === null) {
			...
			for (const message of context.messages) {
				if (typeof message.content === "string") continue;
				for (const part of message.content) if (part.type === "image") images++;
			}
