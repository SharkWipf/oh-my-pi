...
export class CompactionMessageManagerComponent implements Component {
	...
	#restoreAnchor(): void {
		if (this.#scanning || !this.#count()) { this.#selected = -1; return; }
		const source = this.options.source;
		if (this.#initialSelectionPending) { this.#select(this.#count() - 1); return; }
		...
		if (exact >= 0) {
			...
			if (!this.#indices || this.#indices[position] === exact) { this.#select(position); return; }
		}
	...
	#matches(row: CompactionMessageRow): boolean {
		return this.#roles.has(row.role) && this.#states.has(this.#state(row)) &&
			(!this.#query || fuzzyMatch(this.#query, singleLine(typeof row.preview === "function" ? row.preview() : row.preview)).matches);
	}
	#filter(): void {
		...
		if (!this.#query && this.#states.size === STATES.length &&
	...
	#showMenu(title: string, items: readonly SelectItem[], select: (value: string) => void, intro: readonly string[] = [], index = 0): void {
		this.#back();
	...
	#filters(selected = 0): void {
		...
		for (const role of ROLES) items.push({ value: `role:${role}`, label: `${this.#roles.has(role) ? "[x]" : "[ ]"} Role: ${role}` });
		for (const state of STATES) items.push({ value: `state:${state}`, label: `${this.#states.has(state) ? "[x]" : "[ ]"} Stored: ${state}` });
		items.push({ value: "default", label: "Restore default filters" }, { value: "done", label: "Done" });
		this.#showMenu("Filters (independent toggles)", items, value => {
			if (value === "done") { this.#back(); return; }
			if (value === "default") { this.#roles = new Set(["user"]); this.#states = new Set(STATES); }
			else if (value.startsWith("role:")) {
				const role = value.slice(5) as CompactionMessageRole;
				if (this.#roles.has(role)) this.#roles.delete(role); else this.#roles.add(role);
			} else {
				const state = value.slice(6) as CompactionMessageState;
				if (this.#states.has(state)) this.#states.delete(state); else this.#states.add(state);
			}
			this.#filter();
	...
	async #run(action: () => ActionResult, ids?: readonly string[]): Promise<void> {
		try {
			...
			if (!this.#disposed) { this.#notice = `Error: ${error instanceof Error ? error.message : String(error)}`; this.#requestRender(); }
		}
	}
	async #setState(state: CompactionMessageState): Promise<void> {
		const row = this.#row(this.#selected);
		if (!row) { this.#notice = "No selected source. Filters do not change action scope."; return; }
		if (row.limitation) { this.#notice = row.limitation; return; }
		if (this.#pendingStates.has(row.id)) { this.#notice = "Saving this source; wait before editing it again."; return; }
	...
	async #resetAll(): Promise<void> {
		...
		try {
			...
			this.#showMenu("Reset all overrides?", [
				{ value: "no", label: "No — keep overrides" }, { value: "yes", label: `Yes — reset ${reset.count} sources/groups` },
			], value => {
				this.#back();
				if (value === "yes") {
					this.#notice = "Saving captured reset…";
					void this.#run(async () => { const result = await reset.apply(); this.refresh(); return result ?? "Saved"; });
				}
			}, [reset.scope, `${reset.count} non-Auto sources/groups. Includes hidden rows.`, "Preserves tags and settings; newer edits are skipped."]);
		} catch (error) {
			if (!this.#disposed && generation === this.#dialogGeneration) this.#notice = `Cannot capture reset: ${error instanceof Error ? error.message : String(error)}`;
		}
		this.#requestRender();
	}
	#classifierAvailable(): boolean {
		...
		this.#showMenu("Classifier unavailable", [
			{ value: "configure", label: "Configure Model" }, { value: "back", label: "Back" },
		], value => {
			this.#back();
			if (value === "configure") void this.#run(() => this.options.settings("compaction.keepUserMessagesLlmModel"));
		}, [classifier.reason ?? `Model unavailable: ${classifier.model}`]);
		return false;
	}
	#classify(): void {
		const row = this.#row(this.#selected);
		if (!row) { this.#notice = "No selected source to classify."; return; }
		if (row.role !== "user") { this.#notice = "Only real user sources can be classified; non-users have manual state."; return; }
		if (!this.#classifierAvailable()) return;
		if (this.#pendingClassify.has(row.id) || row.status === "queued" || row.status === "running") { this.#notice = "This source is already queued/running."; return; }
		this.#pendingClassify.add(row.id);
		void this.#run(async () => {
			try { return await this.options.classify(row.id); }
			finally { this.#pendingClassify.delete(row.id); this.refresh([row.id]); }
		}, [row.id]);
	}
	#backfill(resume = false): void {
		if (!this.#classifierAvailable()) return;
		const summary = this.options.summary();
		this.#showMenu(resume ? "Resume missing classification?" : "Backfill missing classification?", [
			{ value: "cancel", label: "Cancel — no requests" }, { value: "workers", label: "Choose workers and launch…" },
		], value => {
			this.#back();
		...
		}, [summary.scope, `Model: ${summary.classifier.model}`, "Many model requests/tokens may incur substantial cost.", "Missing/current-unusable facts only; includes hidden rows. Continues after closing this view."]);
	}
	#jobLines(): readonly string[] {
		const summary = this.options.summary();
		const job = summary.job;
		return job ? [summary.scope, `State: ${job.state}`, `Queued ${job.queued} / running ${job.running}`, `Completed ${job.completed} / saved ${job.saved}`, `Failed ${job.failed} / pending ${job.pending ?? job.queued + job.running}`, "Cancel preserves tags and unrelated live work."] : ["No backfill job in this session. Resume starts missing-only work."];
	}
	#job(): void {
		this.#showMenu("Classifier job", [
			{ value: "cancel", label: "Cancel backfill only" }, { value: "resume", label: "Resume missing/current-unusable…" },
			{ value: "refresh", label: "Refresh status" }, { value: "back", label: "Back" },
		], value => {
			if (value === "resume") { this.#backfill(true); return; }
			if (value === "refresh") { this.#job(); return; }
			this.#back();
			if (value === "cancel") {
				const job = this.options.summary().job;
				if (!job || (job.queued === 0 && job.running === 0 && job.state !== "running")) this.#notice = "No active backfill to cancel.";
	...
	#action(action: string): void {
		...
		const row = this.#row(this.#selected);
		switch (action) {
			case "never": case "auto": case "always": void this.#setState(action); break;
			case "reset": void this.#setState("auto"); break;
			case "cycle": if (row) void this.#setState(STATES[(STATES.indexOf(this.#state(row)) + 1) % STATES.length]!); else this.#notice = "No selected source."; break;
			case "inspect":
				if (row) {
			...
			case "filters": this.#filters(); break;
			case "reset-all": void this.#resetAll(); break;
			...
			case "preview": this.#showDocument("Policy / preview", this.options.summary().lines); break;
			case "help": this.#showDocument("Context help", HELP); break;
			case "close": this.dispose(); this.options.close(); break;
		}
		this.#requestRender();
	}
	...
	handleInput(data: string): void {
		...
		if (this.#menu) { this.#menu.handleInput(data); this.#requestRender(); return; }
		if (matchesKey(data, "escape")) {
			if (this.#document?.back) this.#document.back();
			else if (this.#document) this.#back(); else this.#action("close");
			return;
		}
		const delta = matchesKey(data, "up") ? -1 : matchesKey(data, "down") ? 1 :
			matchesKey(data, "pageUp") ? -this.#bodyRows : matchesKey(data, "pageDown") ? this.#bodyRows : 0;
		if (this.#document) {
			if (delta) this.#documentTop = Math.max(0, Math.min(this.#documentTop + delta, this.#documentLength - this.#bodyRows));
	...
	#mouse(event: SgrMouseEvent): boolean {
		if (event.leftClick && event.row === this.#footerRow) {
			...
			if (this.#document) this.#documentTop = Math.max(0, Math.min(this.#documentTop + event.wheel * 3, this.#documentLength - this.#bodyRows));
			else this.#select(this.#selected + event.wheel * 3);
		} else if (!this.#input && !this.#document && event.leftClick && event.row >= this.#bodyStart && event.row < this.#bodyStart + this.#bodyRows) {
			const index = this.#top + event.row - this.#bodyStart;
			if (index < this.#count()) { this.#select(index); this.#actions(); }
		}
		this.#requestRender();
		return true;
	}
	...
	#renderRow(row: CompactionMessageRow, selected: boolean, width: number): string {
		...
		const status = pending ? frames[this.#frame % frames.length] : row.status === "failed" ? "!" : row.status === "succeeded" ? "+" : row.status === "canceled" ? "x" : " ";
		let prefix = `${selected ? ">" : " "}${GLYPHS[state]}${row.hardRecent ? "H" : " "}${row.alwaysAdmitted ? "A" : " "}${status} `;
		if (width >= 48) prefix += `${row.firstIndex === undefined ? "" : `F${row.firstIndex}`}`.padEnd(5) + `${row.recentIndex === undefined ? "" : `R${row.recentIndex}`}`.padEnd(5);
	...
	#syncSpinner(active: boolean): void {
		if (active && !this.#spinner && !this.#disposed) {
			this.#spinner = setInterval(() => { this.#frame++; this.#requestRender(); }, 80);
			this.#spinner.unref();
		} else if (!active && this.#spinner) { clearInterval(this.#spinner); this.#spinner = undefined; }
	}
	render(width: number): readonly string[] {
		...
		const title = this.#menu ? this.#menuTitle : this.#document?.title ?? (this.#inputKind === "workers" ? "Classifier workers" : "Context");
		if (height >= 3) out.push(fit(theme.bold(singleLine(`${title} · ${summary.scope}`))));
		if (height >= 7) out.push(fit(singleLine(this.#notice || summary.lines[0] || "Current policy / next preview ≠ installed representation")));
		if (height >= 9 && !this.#menu && !this.#document) out.push(fit(singleLine(this.#scanning ? `Filtering ${this.#scanAt}/${this.#scanTotal} — Esc closes` :
		...
		if (this.#menu) {
			...
			out.push(...this.#menu.render(width).slice(0, Math.max(0, height - out.length - 1)).map(fit));
			...
			if (!this.#count() && this.#bodyRows) out.push(fit(this.options.source.count("all") ? (this.#scanning ? "Filtering sources…" : "No filter results. a Actions") : "No sources in this session epoch. a Actions"));
		...
		this.#footerHits = [];
		let footer = "";
		for (const [label, action] of [["a Actions", "actions"], ["? Help", "help"], ["Esc Back", "back"], ["/ Search", "search"], ["f Filters", "filters"], ["i Inspect", "inspect"]]) {
