...
export class InputController {
	...
	setupEditorSubmitHandler(): void {
		this.ctx.editor.onSubmit = async (text: string) => {
			...
			const originalSubmission = this.#restoredOriginalSubmission(
				text,
				this.ctx.editor.pendingImages,
				this.ctx.editor.pendingImageLinks,
				compactionOverride,
			) ?? { text: originalText, images: this.ctx.editor.pendingImages.length ? [...this.ctx.editor.pendingImages] : undefined, imageLinks: this.ctx.editor.pendingImageLinks.length ? [...this.ctx.editor.pendingImageLinks] : undefined, compactionOverride };
	...
	async #submitToFocusedSession(
		...
		try {
			...
			this.ctx.editor.setCollapsedText(originalSubmission?.text ?? restoreCompactionOverridePrompt(text, compactionOverride));
			this.ctx.editor.restoredOriginalSubmissions = restoredOriginalSubmissions;
			this.ctx.showError(error instanceof Error ? error.message : String(error));
		}
		this.ctx.updatePendingMessagesDisplay();
	...
	async handleFollowUp(): Promise<void> {
		...
		const originalSubmission = this.#restoredOriginalSubmission(text, images, imageLinks, compactionOverride) ?? { text: originalText, images, imageLinks, compactionOverride };
		const restoredOriginalSubmissions = this.ctx.editor.restoredOriginalSubmissions;
	...
	restoreQueuedMessagesToEditor(options?: { abort?: boolean; currentText?: string }): number {
		...
		].map(entry => entry.originalSubmission ? { ...entry, ...entry.originalSubmission } : entry);
