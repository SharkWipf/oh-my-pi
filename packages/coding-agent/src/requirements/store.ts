...
export function getRequirementsConsumedSources(
	...
	const dynamicLineages = new Set(
		scoped
			.filter(
				revision =>
					revision.lifecycle !== "historical" &&
					!headIds.has(revision.id) &&
					!explicitPredecessors.has(revision.id),
			)
			.map(revision => revision.requirementId),
	);
	for (const revision of scoped)
		if (dynamicLineages.has(revision.requirementId)) {
			revisionIds.add(revision.id);
			batchIds.add(revision.batchId);
		}
	...
	for (const batch of snapshot.batches) {
		...
		const live =
			(batch.status === "pending" || batch.status === "reviewed") &&
			authority.ownerSessionId === context.sessionId &&
			authority.branchId === context.branchId &&
	...
	const coverageGaps = new Map(
		sources
			.filter(source => !source.referenceOnly && source.state !== "complete")
	...
	return {
		sources,
		applicableRevisionIds: heads
			.filter(group => group.length === 1 && group[0].lifecycle === "accepted")
...
function validateReview(
	...
	const accepted = (outcome: RequirementsBatch["review"]["sanity"], label: string) => {
		if (
			...
			outcome.candidates.length !== operationIds.length
		)
		...
		for (const candidate of outcome.candidates) {
			if (
				...
				throw new Error(`${label} candidate decision is absent, duplicated, rejected or uncertain`);
	...
	if (!review.literalAcceptance) {
		...
		for (const obligation of review.evidence.obligations) {
			if (
				!obligation.id ||
				seen.has(obligation.id) ||
				obligation.decision !== "pass" ||
				!obligation.reason.trim() ||
				!obligation.statement.trim() ||
				!obligation.sourceUnitIds.length ||
				obligation.sourceUnitIds.some(id => !source.units.some(unit => unit.id === id && !unit.unsupportedReason))
			)
				throw new Error("Independent obligation is invalid, uncovered, rejected or uncertain");
			seen.add(obligation.id);
			if (!obligation.operationIds.length && !obligation.applicableRevisionIds.length)
				throw new Error("Independent obligation has no candidate or applicable-head coverage");
			if (
				obligation.adoptedUnitIds.some(
					id => !source.adoptedUnitIds?.includes(id) || !obligation.sourceUnitIds.includes(id),
				)
			)
				throw new Error("Obligation claims unaccepted literal coverage");
			for (const id of obligation.operationIds) {
				const operation = byId.get(id);
				if (
					!operation ||
					operation.kind !== obligation.kind ||
					(operation.kind !== "add" && operation.requirementId !== obligation.requirementId) ||
					obligation.predecessorRevisionIds.some(id => !operation.predecessorRevisionIds.includes(id)) ||
					!operation.evidence.some(
						unit => unit.sourceKey === sourceKey && obligation.sourceUnitIds.includes(unit.unitId),
					)
				)
					throw new Error("Candidate does not perform the independently inventoried obligation");
				covered.add(id);
			}
			for (const id of obligation.applicableRevisionIds) {
				const revision = known.get(id),
					heads = revision && readHeads[revision.requirementId];
				if (
					!revision ||
					...
					heads?.length !== 1 ||
					heads[0] !== id ||
					(obligation.kind === "withdraw") !== (revision.kind === "withdraw") ||
					(obligation.kind !== "add" &&
						(revision.requirementId !== obligation.requirementId ||
							obligation.predecessorRevisionIds.includes(id)))
				)
					throw new Error("Obligation does not resolve to a current applicable accepted head");
			}
		}
		if (operationIds.some(id => !covered.has(id)))
			throw new Error("Candidate lacks independently inventoried source-obligation coverage");
		return;
	}
	const literal = review.literalAcceptance;
	if (
		!literal.actor ||
		sourceKey !== literal.sourceKey ||
		source.integrity !== literal.integrity ||
		!literal.unitIds.length ||
		operations.length !== literal.unitIds.length
	)
		throw new Error("Literal acceptance requires complete current units");
	const adopted = new Set<string>();
	for (const operation of operations) {
		if (operation.evidence.length !== 1)
			throw new Error("Literal acceptance requires one complete unit per operation");
		const span = operation.evidence[0],
			unit = source.units.find(item => item.id === span.unitId);
		if (
			!unit ||
			span.sourceKey !== source.key ||
			span.integrity !== source.integrity ||
			!literal.unitIds.includes(unit.id) ||
			adopted.has(unit.id) ||
			unit.byteLength !== Buffer.byteLength(operation.statement) ||
			unit.sha256 !== createHash("sha256").update(operation.statement).digest("hex")
		)
...
class RequirementsReducer {
	...
	constructor(
		readonly snapshot: RequirementsSnapshot = createEmptyRequirementsSnapshot(),
		readonly readSource?: (key: string) => RequirementsSource | undefined,
		readonly readRelations?: (key: string) => RequirementsRelation[],
	) {
	...
	intakeRequirementsSource(source: RequirementsSource): void {
		...
		if (existing) {
			...
			if (!metadataOnly) {
				...
				if (source.state === "unsupported" && existing.state !== "complete" && existing.state !== "gap") {
			...
			if (source.authorityGeneration !== undefined && existing.authorityGeneration === undefined)
				existing.authorityGeneration = source.authorityGeneration;
			return;
		}
	...
	publishRequirementsBatch(
		...
		for (const operation of batch.operations)
			if (
				snapshot.state.tombstones?.some(
					item => sameScope(item.scope, operation.scope) && (source.authorityGeneration ?? -1) < item.generation,
				)
			)
				return result(
					"rejected",
					"Source predates an explicit scoped clear; new accepted operator input is required",
				);
		...
		try {
			...
			for (const operation of batch.operations) {
				...
				if (source.origin.kind !== "human" || !operation.evidence.some(span => span.sourceKey === source.key))
	...
	reconcileRequirementsSources(observations: RequirementsObservation[]): void {
		...
		for (const observation of observations) {
			...
			changed = true;
			affected.add(source.key);
			source.integrityAvailable = observation.integrity !== null;
			if (observation.integrity && observation.integrity !== source.integrity) {
				source.integrity = observation.integrity;
				delete source.adoptedUnitIds;
			}
			...
			source.reason = observation.integrity
				? "Original available; current coverage must be checked"
				: "Original source unavailable";
		}
		const availability = (key: string, integrity: string): RequirementsRevision["availability"] => {
			const source = this.#sources.get(key);
			return !source || source.integrityAvailable === false
				? "unavailable"
		...
		for (const revision of this.snapshot.revisions) {
			const evidence = [
				...revision.evidence,
				...(revision.referents ?? []),
				...(revision.relations ?? []).flatMap(item => item.evidence),
			];
			if (!affected.has(revision.sourceKey) && !evidence.some(item => affected.has(item.sourceKey))) continue;
			const states = [
				availability(revision.sourceKey, revision.sourceIntegrity),
				...evidence.map(item => availability(item.sourceKey, item.integrity)),
			];
			revision.availability = states.includes("unavailable")
				? "unavailable"
			...
			if (dependent?.state === "complete" && revision.availability !== "available") {
				dependent.state = "pending";
				dependent.reason = "Referenced original evidence is unavailable or changed";
			}
		}
		for (const batch of this.snapshot.batches) {
			if (!affected.has(batch.sourceKey) && !Object.keys(batch.readSourceIntegrities).some(key => affected.has(key)))
				continue;
			if (batch.status !== "accepted") {
				batch.status = "stale";
				batch.reason = "Frozen source evidence changed or became unavailable";
				continue;
			}
			const source = this.#sources.get(batch.sourceKey);
			if (
				source &&
				availability(source.key, batch.sourceIntegrity) === "available" &&
				(!batch.review.literalAcceptance || source.units.every(unit => source.adoptedUnitIds?.includes(unit.id))) &&
				Object.entries(batch.readSourceIntegrities).every(
					([key, integrity]) => availability(key, integrity) === "available",
				)
			) {
	...
	restoreRequirements(receipt: RequirementsRestoreReceipt): boolean {
		...
		try {
			if (receipt.generation !== this.snapshot.state.generation || !readHeadsMatch(this.snapshot, receipt.readHeads))
			...
			validateReview(
				this.#sources,
				receipt.review,
				revisions as RequirementsRevision[],
				receipt.operationIds,
				revisions[0]!.sourceKey,
				this.snapshot.revisions,
				receipt.readHeads,
			);
		} catch (error) {
			for (const revision of revisions)
...
function hasSourceRelation(
	...
	const relations = [
		...snapshot.revisions
			.filter(item => item.lifecycle === "accepted" && (!item.availability || item.availability === "available"))
...
function applicableHeads(heads: RequirementsRevision[][]): Pick<RequirementsApplicable, "active" | "conflicts"> {
	...
	for (const group of heads) {
		if (group.length > 1) conflicts.push(group);
			...
			group.length === 1 &&
			group[0].lifecycle === "accepted" &&
			(!group[0].availability || group[0].availability === "available") &&
			group[0].kind !== "withdraw"
		)
			active.push(group[0]);
	}
...
export function getApplicableRequirements(
	...
	return {
		...
		coverageGaps: snapshot.sources.filter(
			source =>
		...
		const heads = rows.filter(
			row =>
				!rows.some(
					other =>
						other.id !== row.id &&
						sameScope(other.scope, row.scope) &&
						(other.predecessorRevisionIds.includes(row.id) ||
							hasSourceRelation(snapshot, row.sourceKey, other.sourceKey, other.relations)),
				),
		);
...
class InMemoryRequirementsBackend implements RequirementsBackend {
	...
	constructor(snapshot = createEmptyRequirementsSnapshot()) {
		...
		for (const revision of snapshot.revisions) {
			...
			rows.push(revision);
			groups.set(revision.requirementId, rows);
		}
	...
	state(
		...
		return structuredClone({
			publicationRevision: this.#state.publicationRevision,
			generation: this.#state.generation,
			owners:
				ownerSessionId && this.#state.owners[ownerSessionId]
					? { [ownerSessionId]: this.#state.owners[ownerSessionId] }
					: {},
			restoreReceipts: receiptId ? this.#state.restoreReceipts.filter(item => item.id === receiptId) : [],
			restoreReviews: Object.fromEntries(
				revisionIds.filter(id => this.#state.restoreReviews[id]).map(id => [id, this.#state.restoreReviews[id]]),
			),
			tombstones: scopes.flatMap(scope =>
				(this.#state.tombstones ?? []).filter(item => sameScope(item.scope, scope)),
			),
		});
	}
	writeState(before: RequirementsSnapshot["state"], after: RequirementsSnapshot["state"]): void {
		this.#state.publicationRevision = after.publicationRevision;
		this.#state.generation = after.generation;
		...
		for (const receipt of after.restoreReceipts)
			if (!this.#state.restoreReceipts.some(item => item.id === receipt.id))
				this.#state.restoreReceipts.push(structuredClone(receipt));
	...
	revisionIdsForBatch(batchId: string): string[] {
		return [...(this.#batchRevisions.get(batchId) ?? [])];
	}
	putSource(source: RequirementsSource): void {
		const old = this.#sources.get(source.key);
		if (old) {
			if (old.referenceOnly) this.#coverage.referenceOnly--;
			else this.#coverage.byState[old.state]--;
			this.#index(this.#pending, old.ownerSessionId, old.key, false);
		} else this.#coverage.total++;
		if (source.referenceOnly) this.#coverage.referenceOnly++;
		else this.#coverage.byState[source.state]++;
		this.#sources.set(source.key, structuredClone(source));
		if (!source.referenceOnly && source.state !== "complete" && source.origin.kind === "human")
			this.#index(this.#pending, source.ownerSessionId, source.key, true);
	}
	#edges(row: RequirementsBatch | RequirementsRevision, add: boolean): void {
		const keys = new Set([row.sourceKey]);
		if ("operations" in row) {
			for (const key of Object.keys(row.readSourceIntegrities)) keys.add(key);
			for (const operation of row.operations) addOperationSourceKeys(operation, keys);
		} else addOperationSourceKeys(row, keys);
		for (const key of keys)
			this.#index(this.#dependencies, ("operations" in row ? "batch" : "revision") + key, row.id, add);
	}
	putBatch(batch: RequirementsBatch): void {
		const old = this.#batches.get(batch.id);
		if (old) this.#edges(old, false);
		this.#batches.set(batch.id, structuredClone(batch));
		this.#index(this.#batchSources, batch.sourceKey, batch.id, true);
		this.#edges(batch, true);
	}
	putRevision(revision: RequirementsRevision): void {
		const old = this.#revisions.get(revision.id);
		if (old) {
			this.#edges(old, false);
			this.#index(this.#batchRevisions, old.batchId, old.id, false);
			for (const relation of old.relations ?? [])
				this.#index(this.#relationRevisions, relation.successorSourceKey, old.id, false);
		}
		this.#revisions.set(revision.id, structuredClone(revision));
		this.#index(this.#batchRevisions, revision.batchId, revision.id, true);
		this.#edges(revision, true);
		if (revision.lifecycle === "accepted" && (!revision.availability || revision.availability === "available"))
			for (const relation of revision.relations ?? [])
				this.#index(this.#relationRevisions, relation.successorSourceKey, revision.id, true);
	}
	relationsForSuccessor(key: string): RequirementsRelation[] {
		return structuredClone(
			[...(this.#relationRevisions.get(key) ?? [])].flatMap(
				id => this.#revisions.get(id)!.relations?.filter(relation => relation.successorSourceKey === key) ?? [],
			),
		);
	}
	deleteBatch(id: string): void {
		const old = this.#batches.get(id);
		if (!old) return;
		this.#edges(old, false);
		this.#index(this.#batchSources, old.sourceKey, id, false);
		this.#batches.delete(id);
	}
	batchIdsForSource(key: string) {
		return [...(this.#batchSources.get(key) ?? [])];
	}
	dependentIds(key: string, kind: "revision" | "batch") {
		return [...(this.#dependencies.get(kind + key) ?? [])];
	}
	#scope(scope: RequirementsScope): string {
		return JSON.stringify([scope.kind, scope.projectId ?? null, scope.sessionId ?? null, scope.epoch ?? null]);
	}
	headIds(context?: RequirementsConsumptionContext, requirementId?: string): string[] {
		...
		return scopes
			.flatMap(scope => [...(this.#scopeHeads.get(this.#scope(scope)) ?? [])])
			.filter(id => requirementsScopeApplies(this.#revisions.get(id)!, context));
	}
	setHeads(requirementId: string, rows: RequirementsRevision[]): void {
		for (const id of this.#heads.get(requirementId) ?? []) {
			const row = this.#revisions.get(id)!;
			this.#index(this.#scopeHeads, this.#scope(row.scope), id, false);
		}
	...
	pending(context?: RequirementsConsumptionContext): RequirementsSource[] {
		const ids = context
			? (this.#pending.get(context.sessionId) ?? [])
			: [...this.#pending.values()].flatMap(ids => [...ids]);
		return [...ids]
			.map(id => this.source(id)!)
			.filter(
				source =>
					!context ||
					(source.epoch === context.epoch && (!context.sourceKeys || context.sourceKeys.has(source.key))),
			);
	}
	coverage(keys?: ReadonlySet<string>): RequirementsCoverageSummary {
		...
		for (const key of keys) {
			...
			if (!source) continue;
			summary.total++;
			if (source.referenceOnly) summary.referenceOnly++;
			else summary.byState[source.state]++;
		}
		return summary;
	}
	snapshot(): RequirementsSnapshot {
...
export class RequirementsStore {
	...
	getRequirementsSnapshot() {
		return this.#backend.transaction(() => this.#backend.snapshot());
	}
	getRequirementsSource(key: string) {
		return this.#backend.source(key);
	}
	getRequirementsBatch(id: string) {
		return this.#backend.batch(id);
	}
	...
	getRequirementsRevision(id: string) {
		return this.#backend.revision(id);
	}
	getRequirementsSourceMetadata(key: string): RequirementsSourceMetadata | undefined {
		const source = this.#backend.source(key);
		if (!source) return undefined;
		return {
			key,
			integrity: source.integrity,
			integrityAvailable: source.integrityAvailable,
			state: source.state,
			locators: source.locators,
		};
	}
	getRequirementsCoverageSummary(keys?: ReadonlySet<string>) {
		return this.#backend.coverage(keys);
	}
	getRequirementsPendingSources(context?: RequirementsConsumptionContext) {
		return this.#backend.pending(context);
	}
	#load(work: RequirementsWork): RequirementsSnapshot {
		const sourceKeys = new Set(work.sourceKeys),
			batchIds = new Set(work.batchIds),
			revisionIds = new Set(work.revisionIds);
		for (const id of work.requirementIds ?? [])
			for (const head of this.#backend.headIds(undefined, id)) revisionIds.add(head);
		...
		for (const id of batchIds) {
			const batch = this.#backend.batch(id);
			if (!batch) continue;
			snapshot.batches.push(batch);
			sourceKeys.add(batch.sourceKey);
			if (batch.status === "accepted")
				for (const revisionId of this.#backend.revisionIdsForBatch(batch.id)) revisionIds.add(revisionId);
			for (const key of Object.keys(batch.readSourceIntegrities)) sourceKeys.add(key);
			for (const requirementId of Object.keys(batch.readHeads ?? {}))
				for (const head of this.#backend.headIds(undefined, requirementId)) revisionIds.add(head);
			for (const operation of batch.operations) {
				addOperationSourceKeys(operation, sourceKeys);
				for (const predecessor of operation.predecessorRevisionIds) revisionIds.add(predecessor);
			}
		}
		for (const id of revisionIds) {
			const revision = this.#backend.revision(id);
			if (!revision) continue;
			snapshot.revisions.push(revision);
			sourceKeys.add(revision.sourceKey);
			addOperationSourceKeys(revision, sourceKeys);
		}
		...
		const scopes = [
			...(work.scopes ?? []),
			...snapshot.batches.flatMap(batch => batch.operations.map(operation => operation.scope)),
			...snapshot.revisions.map(revision => revision.scope),
		];
	...
	#mutate<T>(work: RequirementsWork, apply: (store: RequirementsReducer) => T): T {
		return this.#backend.transaction(() => {
			...
			const result = apply(
				new RequirementsReducer(
					snapshot,
					key => this.#backend.source(key),
					key => this.#backend.relationsForSuccessor(key),
				),
			);
			...
			for (const source of snapshot.sources)
				if (previousSources.get(source.key) !== JSON.stringify(source)) this.#backend.putSource(source);
			for (const batch of snapshot.batches) {
				if (previousBatches.get(batch.id) !== JSON.stringify(batch)) this.#backend.putBatch(batch);
				previousBatches.delete(batch.id);
			}
			for (const id of previousBatches.keys()) this.#backend.deleteBatch(id);
			const changedRequirements = new Set<string>();
			for (const revision of snapshot.revisions)
				if (previousRevisions.get(revision.id) !== JSON.stringify(revision)) {
					this.#backend.putRevision(revision);
					changedRequirements.add(revision.requirementId);
				}
			for (const id of changedRequirements) {
				const rows = new Map(
					this.#backend.headIds(undefined, id).map(head => [head, this.#backend.revision(head)!]),
				);
				for (const revision of snapshot.revisions)
					if (revision.requirementId === id) rows.set(revision.id, revision);
				const candidates = [...rows.values()];
				this.#backend.setHeads(
					id,
					candidates.filter(
						row =>
							!candidates.some(
								other =>
									other.id !== row.id &&
									sameScope(row.scope, other.scope) &&
									(other.predecessorRevisionIds.includes(row.id) ||
										hasSourceRelation(
											snapshot,
											row.sourceKey,
			...
			if (JSON.stringify(before.state) !== JSON.stringify(snapshot.state))
				this.#backend.writeState(before.state, snapshot.state);
			return result;
		});
	}
	getRequirementsConsumptionSnapshot(context: RequirementsConsumptionContext): RequirementsConsumptionSnapshot {
		return this.#backend.transaction(() => {
			...
			for (const id of headIds) {
				const row = snapshot.revisions.find(row => row.id === id)!;
				const group = heads.get(row.requirementId) ?? [];
				group.push(row);
				heads.set(row.requirementId, group);
			}
			const groups = [...heads.values()];
			return {
				...snapshot,
				readHeads: Object.fromEntries([...heads].map(([id, rows]) => [id, rows.map(row => row.id)])),
				coverage: this.#backend.coverage(),
				pendingSources: [],
				applicable: {
					...applicableHeads(groups),
					coverageGaps: snapshot.sources.filter(source => !source.referenceOnly && source.state !== "complete"),
				},
				applicableRevisionIds: groups
					.filter(
						rows =>
							rows.length === 1 &&
							rows[0].lifecycle === "accepted" &&
							(!rows[0].availability || rows[0].availability === "available"),
					)
					.map(rows => rows[0].id),
			};
		});
	...
	authorizeRequirementsOwner(authority: RequirementsAuthority): void {
		this.#mutate({ ownerSessionId: authority.ownerSessionId }, store => store.authorizeRequirementsOwner(authority));
	}
	invalidateRequirementsOwner(ownerSessionId: string): void {
		this.#mutate({ ownerSessionId }, store => store.invalidateRequirementsOwner(ownerSessionId));
	}
	intakeRequirementsSource(source: RequirementsSource): void {
	...
	setRequirementsSourceDisposition(
		...
		this.#mutate({ sourceKeys: [key] }, store =>
			store.setRequirementsSourceDisposition(key, integrity, state, reason),
		);
	}
	recordRequirementsGap(key: string, integrity: string, actor: string, reason: string): void {
		this.#mutate({ sourceKeys: [key] }, store => store.recordRequirementsGap(key, integrity, actor, reason));
	}
	saveRequirementsBatch(batch: RequirementsBatch): string {
		return this.#mutate(
			{ sourceKeys: [batch.sourceKey], batchIds: this.#backend.batchIdsForSource(batch.sourceKey) },
			store => store.saveRequirementsBatch(batch),
		);
	}
	publishRequirementsBatch(
		batchId: string,
		...
		return this.#mutate({ batchIds: [batchId], ownerSessionId: authority.ownerSessionId }, store =>
			store.publishRequirementsBatch(batchId, authority, verifiedIntegrities),
		);
	}
	reconcileRequirementsSources(observations: RequirementsObservation[]): void {
		const latest = new Map<string, RequirementsObservation>();
		for (const observation of observations)
			latest.set(observation.key, { ...latest.get(observation.key), ...observation });
		const changed = [...latest.values()].filter(observation => {
			const source = this.#backend.source(observation.key);
			return (
				source &&
				!(
					observation.integrity === source.integrity &&
					source.integrityAvailable !== false &&
					(!observation.locators || JSON.stringify(source.locators) === JSON.stringify(observation.locators))
				) &&
				!(observation.integrity === null && source.integrityAvailable === false && !observation.locators)
			);
		});
		if (changed.length) {
	...
	quarantineRequirements(revisionIds: string[], actor: string, reason: string): number {
		return this.#mutate({ revisionIds }, store => store.quarantineRequirements(revisionIds, actor, reason));
	}
	restoreRequirements(receipt: RequirementsRestoreReceipt): boolean {
		return this.#mutate(
			{
				revisionIds: receipt.revisionIds,
				requirementIds: Object.keys(receipt.readHeads ?? {}),
				receiptId: receipt.id,
			},
			store => store.restoreRequirements(receipt),
		);
	}
	withdrawRequirements(revisionIds: string[], actor: string, reason: string): number {
		...
		return this.#mutate({ revisionIds }, store => {
			const revisions = revisionIds.map(id => {
				const revision = store.snapshot.revisions.find(row => row.id === id);
				if (!revision) throw new Error(`Unknown revision: ${id}`);
				return revision;
			});
			...
			for (const revision of revisions) {
				revision.lifecycle = "historical";
				revision.withdrawal = { actor, reason, generation };
			}
	...
	clearRequirements(scope: RequirementsScope, actor: string, reason: string): number {
		validateScope(scope);
		if (!actor || !reason) throw new Error("Scoped clear requires actor and reason");
		const context = {
		...
		return this.#mutate({ revisionIds: ids, scopes: [scope] }, store => {
			const generation = ++store.snapshot.state.generation;
			for (const revision of store.snapshot.revisions)
				if (sameScope(revision.scope, scope)) {
					revision.lifecycle = "historical";
					revision.quarantine = { actor, reason, generation };
				}
			store.snapshot.state.tombstones ??= [];
			store.snapshot.state.tombstones = store.snapshot.state.tombstones.filter(
				item => !sameScope(item.scope, scope),
			);
			store.snapshot.state.tombstones.push({ scope, actor, reason, generation });
			store.snapshot.state.publicationRevision++;
			return generation;
		});
	}
