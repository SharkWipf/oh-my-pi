...
import * as artifactsModule from "@oh-my-pi/pi-coding-agent/session/artifacts";
...
describe("AgentSession shake", () => {
	...
	it("rejects an artifact plan when its selected source changes during the save", async () => {
		...
		const save = artifactsModule.writeArtifact;
		vi.spyOn(artifactsModule, "writeArtifact").mockImplementation(async (...args) => {
			started.resolve();
