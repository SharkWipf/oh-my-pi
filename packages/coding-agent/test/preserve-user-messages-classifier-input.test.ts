...
const image: ImageContent = {
	type: "image",
	mimeType: "image/png",
	data: "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+aMwoAAAAASUVORK5CYII=",
};
...
describe("classifier source projection", () => {
	it("keeps actual empty and image-only previous users in every builder", () => {
		...
		expect(buildPreservedUserMessageClassifierInput(entries, "2")!.previousUserMessage).toEqual([
			{ type: "text", text: "" },
		]);
	...
	it("counts two assistant messages, retains all calls, excludes results/reasoning, snapshots arguments", () => {
		...
		const entries = branch([
			...
			assistant([
				{ type: "toolCall", id: "a", name: "read", arguments: args },
			...
			]),
			{
				...
				toolCallId: "a",
				toolName: "read",
				content: [{ type: "text", text: "excluded output" }],
				isError: false,
				timestamp: 1,
			},
		...
		expect(
			preservedUserMessageClassifierInputsEqual(input, buildPreservedUserMessageClassifierInput(entries, "6")),
		).toBe(false);
		expect(input.previousAssistantMessages[0]![0]).toEqual({
			type: "toolCall",
			name: "read",
	...
	it("resets relative context at clear and ignores synthetic users", () => {
		const entries = branch([
			user("old"),
			user("ignored"),
			{ ...user("continue"), synthetic: true } as AgentMessage,
			user("new"),
		]);
...
describe("classifier request boundary", () => {
	it("sends complete current text beyond title limits and original images", () => {
		...
		const request = buildPreservedUserMessageClassifierRequest(
			buildPreservedUserMessageClassifierInput(entries, "0")!,
			model,
		);
	...
	it("bounds auxiliaries without truncating current and rejects oversized current", () => {
		...
		const request = buildPreservedUserMessageClassifierRequest(
			buildPreservedUserMessageClassifierInput(entries, "1")!,
			constrained,
		);
		...
		expect(() =>
			buildPreservedUserMessageClassifierRequest(
				buildPreservedUserMessageClassifierInput(branch([user("target ".repeat(20000))]), "0")!,
				constrained,
			),
		).toThrow("complete current message exceeds");
	});
	...
	it("does not assign zero to image-only or unsupported targets", () => {
		...
		expect(() => buildPreservedUserMessageClassifierRequest(input, { ...model, input: ["text"] })).toThrow(
			"does not support",
		);
	...
	it("requires every known category and rejects unknown or nonboolean fields", () => {
		expect(
			parsePreservedUserMessageCategoryMask(JSON.stringify({ ...allFalseCategories, question: undefined })),
		).toBeUndefined();
		expect(
			parsePreservedUserMessageCategoryMask(JSON.stringify({ ...allFalseCategories, unknown: false })),
		).toBeUndefined();
		expect(
			parsePreservedUserMessageCategoryMask(JSON.stringify({ ...allFalseCategories, question: "false" })),
		).toBeUndefined();
		expect(
			parsePreservedUserMessageCategoryMask(JSON.stringify({ ...allFalseCategories, question: 0 })),
		).toBeUndefined();
	});
	...
	it("rejects interrupted output even when its text looks complete", async () => {
		...
		await expect(
			classifyPreservedUserMessage(input, {
				model,
				complete: async () => ({
					...assistant([{ type: "text", text: JSON.stringify(allFalseCategories) }]),
					stopReason: "length",
				}),
			}),
...
describe("bounded classifier scanning and validation", () => {
	it("validates only the four named inputs and detects changed source bodies", () => {
		...
		let lookups = 0;
		expect(
			matchesPreservedUserMessageClassifierSources(input, id => {
				lookups++;
				return byId.get(id);
			}),
		).toBe(true);
	...
	it("keeps cooperative source-neighbor projection identical across empty users, tool calls and resets", async () => {
		...
		const actual = [];
		for await (const target of iteratePreservedUserMessageClassifierInputsCooperatively(entries, () => true))
			actual.push(target);
