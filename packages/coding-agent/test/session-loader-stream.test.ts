...
it("point reads preserve top-level IDs, whole UTF-8 records, and fresh rewritten/appended content", async () => {
	...
	const targetLine =
		...
		',"\\u0069d":' +
		encodedId +
		',"type":"message","parentId":"decoy","timestamp":' +
		JSON.stringify(ISO) +
		"}";
	...
	const file = await writeTemp(
		[
			...
			'{"id":' + JSON.stringify(id) + ',"broken":}',
			targetLine,
		].join("\n"),
	);
	const selected = new Set(["decoy", id, "phantom"]);
	let entries: FileEntry[] = [];
	await sessionLoader.visitEntriesFromFile(
		file,
		entry => {
			entries.push(entry);
		},
	...
	await storage.writeTextAtomic(
		file,
		[slot, JSON.stringify(HEADER), JSON.stringify(msg(id, "s1", "Rewritten original")), ""].join("\n"),
	);
	entries = [];
	await sessionLoader.visitEntriesFromFile(
		file,
