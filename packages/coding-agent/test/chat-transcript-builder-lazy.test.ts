...
import { ChatTranscriptBuilder, type ChatTranscriptBuilderDeps } from "@oh-my-pi/pi-tui/chat/chat-transcript-builder";
import { appendOutlineEntries, outlineVisibility } from "@oh-my-pi/pi-tui/chat/transcript-outline";
...
let previousPreferences: typeof chatTranscriptDisplayPreferences;
beforeEach(async () => {
	previousPreferences = { ...chatTranscriptDisplayPreferences };
	setChatTranscriptDisplayPreferences({ showTokenUsage: true, showTurnTime: true, cacheMissMarker: true });
	await initTheme();
});
afterEach(() => {
	for (const instance of builders.splice(0)) instance.dispose();
	setChatTranscriptDisplayPreferences(previousPreferences);
});
...
describe("source-backed lazy transcript replay", () => {
	...
	it("keeps rewind source identities stable when prior hub and todo panels disappear", () => {
		setChatTranscriptDisplayPreferences({ showTokenUsage: false });
		const lazy = builder(true);
