...
describe("message classifier session jobs", () => {
	...
	beforeEach(() => {
		...
		auth.keys.setRuntime(model.provider, "synthetic-key");
	...
	it("classifies with a configured local TINY model without falling back after failure", async () => {
		const id = user("synthetic local tiny source");
		...
		expect(availability).toEqual({ available: true, model: "local/lfm2.5-230m" });
		const failed = await session.startMessageClassification(id);
		await until(() => provider.requests.length === 1);
		expect(job(failed).model).toBe("local/lfm2.5-230m");
		provider.requests[0]!.finish("invalid classifier response");
		await settled(failed);
		expect(job(failed).state).toBe("failed");
		expect(provider.requests).toHaveLength(1);
		...
		provider.requests[1]!.finish();
		await settled(retry);
		expect(job(retry).model).toBe("local/lfm2.5-230m");
		expect((await facts(await reopen())).get(id)).toBe(0);
	});

	it("rejects a non-language model without invoking a completion provider", async () => {
		const id = user("synthetic wrong model kind");
		session.settings.override("compaction.keepUserMessagesLlmModel", "local/whisper-base");
		expect((await session.getMessageClassificationAvailability()).available).toBe(false);
		await expect(session.startMessageClassification(id)).rejects.toThrow();
		expect(provider.requests).toEqual([]);
		expect((await facts()).has(id)).toBe(false);
	});

	it("holds a completed classifier result through session-switch rollback reconciliation", async () => {
		const id = user("synthetic rollback classifier source");
		const started = await session.startMessageClassification(id);
		await until(() => provider.requests.length === 1);
		const originalSessionId = manager.getSessionId();
		const targetCwd = dir.join("switch-target");
		await mkdir(targetCwd);
		const target = SessionManager.create(targetCwd, dir.path());
		managers.push(target);
		target.appendMessage({ role: "user", content: "synthetic switch target", timestamp: 1 });
		await target.ensureOnDisk();
		await target.flush();
		const targetFile = target.getSessionFile();
		if (!targetFile) throw new Error("Missing persisted switch target");
		...
		const failure = new Error("synthetic settings adoption failure");
		...
		try {
			...
			expect((await facts(await reopen())).get(id)).toBe(0);
			expect((await facts(target)).has(id)).toBe(false);
		} finally {
			failAdoption.resolve();
			rollbackRelease.resolve();
