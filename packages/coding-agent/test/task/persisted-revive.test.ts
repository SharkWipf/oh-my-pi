...
describe("persisted subagent revival", () => {
	...
	describe("wake-turn relay", () => {
		...
		it.each(["killed", "replaced"] as const)(
			"fences late success from a %s wake owner but delivers one cancellation",
			async state => {
				...
				expect(
					(await bus.send({ from: ref.id, to: "Main", body: "Unauthorized late message", wakeRelay: true }))
						.outcome,
				).toBe("failed");
