...
describe("indexed session context", () => {
	...
	it("retains notes and the initiating request outside a rollover suffix across branches and resets", () => {
		const session = SessionManager.inMemory();
		session.appendMessage({ role: "user", content: "finish the original request", timestamp: 1 });
		session.appendCustomEntry("experimental_context_notes", { version: 1, text: "retained notebook" });
		session.appendCustomEntry("experimental_context_notes", { version: 99, text: "invalid notebook" });
		const kept = session.appendMessage(makeAssistantMessage());
		const boundary = session.appendCompaction("rollover", undefined, kept, 100, {
			details: { kind: "experimental-context-rollover", version: 1 },
		});
		const first = session.appendMessage({ role: "user", content: "new request", timestamp: 2 });
		const context = session.buildSessionContext();
		expect(context).toEqual(buildSessionContext(session.getEntries(), first));
		expect(context.messages).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ role: "user", content: "finish the original request" }),
				expect.objectContaining({ role: "custom", content: expect.stringContaining("retained notebook") }),
			]),
		);
		session.branch(boundary);
		session.appendCustomEntry("experimental_context_notes", { version: 1, text: "" });
		expect(session.buildSessionContext()).toEqual(buildSessionContext(session.getEntries(), session.getLeafId()));
		session.branch(first);
