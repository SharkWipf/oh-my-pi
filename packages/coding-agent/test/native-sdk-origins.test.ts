...
import { normalizeModelContextMessages } from "../src/utils/image-loading";
...
function parts(value: object) {
	const origin = getSourceOrigin(value);
	expect(origin?.kind).toBe("source");
	if (origin?.kind !== "source") throw new Error("Missing source origin");
	return origin.parts;
}
...
describe("native SDK transform origins", () => {
	it("keeps equal-text IDs distinct through cloning and secret replacement without invented original offsets", () => {
		...
		const output = obfuscateMessages(
			new SecretObfuscator([{ type: "plain", content: secret }]),
			cloneWithSourceOrigins(messages),
		);
	...
	it("redacts typed provider-visible metadata without changing opaque replay state", () => {
				...
				content: [
					{
						...
						providerMetadata: {
							...
							actions: [
								{ type: "type", text },
								{ type: "keypress", keys: [secret] },
							],
							pendingSafetyChecks: [{ id: secret, code: secret, message: text }],
						},
					},
					{
						type: "anthropicServerTool",
						block: { type: "server_tool_use", id: secret, name: "web_search", input: { query: text } },
					},
					{
						type: "anthropicServerTool",
						block: {
					...
					items: [
						{
							...
							action: {
								type: "search",
								queries: [text],
								sources: [{ type: "url", url: `https://example.invalid/${secret}` }],
							},
							opaque,
						},
						{ type: "reasoning", id: secret, encrypted_content: secret },
					],
				},
			},
			{
				...
				timestamp: 2,
				content: [],
				details: { explanation: text, opaque },
				providerMetadata: {
					type: "computer",
					screenshot: { type: "computer_screenshot", file_id: secret },
					acknowledgedSafetyChecks: [{ id: secret, code: secret, message: text }],
				},
			},
		];
		...
		if (
			call.type !== "toolCall" ||
			server.type !== "anthropicServerTool" ||
			server.block.type !== "server_tool_use" ||
			search.type !== "anthropicServerTool" ||
			!Array.isArray(search.block.content)
		)
			throw new Error("Expected typed metadata");
		...
		const visible = [
			call.arguments.text,
			call.providerMetadata!.actions[0]!.type === "type" ? call.providerMetadata!.actions[0]!.text : "",
			call.providerMetadata!.pendingSafetyChecks[0]!.message,
			server.block.input!.query,
			search.block.content[0].title,
			result.providerMetadata!.acknowledgedSafetyChecks[0]!.message,
			details.explanation,
			action.queries[0],
		];
		...
		expect(parts(result).find(part => part.blockIndex === "metadata.screenshot")?.representation).toBe(
			"original-image",
		);
	...
	it("keeps per-source image identity through real encoding, cached normalization and URL round trips", async () => {
		...
		const messages = [
			user([{ type: "image", data: webp, mimeType: "image/webp" }]),
			user([{ type: "image", data: webp, mimeType: "image/webp" }]),
		];
	...
	it("removes only omitted image coverage in generic and native content", async () => {
		...
		const nativeOrigin = {
			...
			parts: parts(message).map((part, index) => ({ ...part, transportBlockIndex: index })),
		};
		const nativeMessage = {
			role: "user" as const,
			content: "kept",
			timestamp: 0,
			providerPayload: { type: "openaiResponsesHistory" as const, items: [native], origins: [nativeOrigin] },
		};
		...
		const many = user(
			Array.from({ length: 101 }, () => ({ type: "image" as const, data: PNG, mimeType: "image/png" })),
		);
	...
	it("counts reminder bytes as controls, not newly authored source text", () => {
		...
		expect((output.messages[0]!.content as string).slice(part.transportSpan!.start, part.transportSpan!.end)).toBe(
			"original",
		);
...
it("honors actual SDK context and provider hooks once, without retaining unsupported attribution", async () => {
	...
	auth.keys.setRuntime(requestModel.provider, "local-only");
	let contextCalls = 0;
	let payloadCalls = 0;
...
it("retains selected originals beyond the ordinary cap, never synthetic raster or former selections", () => {
	...
	expect(output.messages[0]!.content).toEqual(selected.content);
	...
	expect(next.messages[1]!.content).toEqual(unselected.content);
});

it("does not grant current-selection immunity to historical or unknown image snapshots", () => {
	const selected = user(
		Array.from({ length: 102 }, () => ({ type: "image" as const, data: PNG, mimeType: "image/png" })),
	);
	bindMessageSource(selected, "selected", 0);
	...
	const output = clampProviderContextImages(
		{ messages: [selected] },
