...
test("next-real legacy migration preserves installed history and reports unavailable original sources", async () => {
	...
	auth.keys.setRuntime("openai", "isolated-no-provider");
	...
	const noProvider = () => {
		throw new Error("Local prerequisite failure must not call a provider");
	};
	try {
		for (let index = 0; index < 8; index++) {
			seed.appendMessage({
				role: "user",
				content: `${index}: ${"Original archived history. ".repeat(300)}`,
				timestamp: index,
			});
		}
		seed.appendMessage({ role: "user", content: "Original retained tail", timestamp: 9 });
		const seedAgent = new Agent({
			initialState: { model, messages: seed.buildSessionContext().messages, systemPrompt: [], tools: [] },
			streamFn: noProvider,
		});
		const preparation = prepareCompaction(
			seed.getBranch(),
			resolveMethodSettings(settings.getGroup("compaction"), "snapcompact"),
			model,
			seedAgent.tokenizer,
		);
		assert(preparation);
		const rendered = await compactArchive(preparation, {
			convertToLlm,
			model,
			shape: resolveShape(model, "8x13-bw"),
			maxFrames: 3,
		});
		...
		// Recorded legacy storage: known PNG identity, unknown aggregate, no current response target.
		...
		for (const kind of ["source-boundary", "original-image"] as const) {
			for (const trigger of ["manual", "automatic"] as const) {
				...
				const importedId: string = manager.appendCompaction(
					rendered.summary,
					rendered.shortSummary,
					boundary,
					rendered.tokensBefore,
					{
				...
				for (let turn = 0; turn < 8; turn++) {
					manager.appendMessage({
						role: "user",
						content: `Fresh turn ${turn}: ${"ordinary new history ".repeat(2000)}`,
						timestamp: 11 + turn,
					});
				}
				...
				try {
					...
					assert(
						typeof diagnostic === "string" && diagnostic.includes(missingId),
						`${kind}/${trigger} must identify the unavailable durable source`,
					);
