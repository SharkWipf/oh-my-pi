...
function sourceKeys(items: readonly object[]): string[][] {
	return exportItemOrigins(items).map(origin =>
		origin.kind === "source" ? origin.parts.map(part => `${part.entryId}:${part.blockIndex}`) : [origin.kind],
	);
}
...
test("Codex emits block-specific origins through one-to-many calls, image filtering and escaping without wire fields", () => {
	...
	const escaped = convertCodexResponsesMessages(createCodexModel("gpt-oss-120b"), {
		messages: [bound({ role: "user", content: "<|channel|>analysis", timestamp: 1 }, "escape", 0)],
	});
	expect(JSON.stringify(escaped)).not.toContain("<|channel|>");
	expect(getSourceOrigin(escaped[0]!)).toMatchObject({
		kind: "source",
		parts: [{ entryId: "escape", coverage: "full", representation: "transformed-text" }],
	});
...
test("three-item prefix plus full snapshot stays three; equal-content distinct IDs and appended delta survive", async () => {
	const sources = [0, 1, 2].map(index =>
		bound({ role: "user" as const, content: "equal", timestamp: 1 }, `u${index}`, index),
	);
	const prefix = convertCodexResponsesMessages(model, { messages: sources });
	const snapshot = assistant([]);
	snapshot.providerPayload = createOpenAIResponsesHistoryPayload(
		model.provider,
		cloneWithSourceOrigins(prefix) as unknown as Record<string, unknown>[],
		false,
	);
	...
	const deltaItems = convertCodexResponsesMessages(model, {
		messages: [bound(assistant([{ type: "text", text: "new" }]), "a", 3)],
	});
	const delta = assistant([]);
	delta.providerPayload = createOpenAIResponsesHistoryPayload(
		model.provider,
		deltaItems as unknown as Record<string, unknown>[],
	);
	...
	legacy.providerPayload = {
		type: "openaiResponsesHistory",
		provider: model.provider,
		items: JSON.parse(JSON.stringify(prefix)),
	};
	expect(sourceKeys(convertCodexResponsesMessages(model, { messages: [legacy] }))).toEqual([
		["unknown"],
		["unknown"],
		["unknown"],
	]);
...
test("full snapshot discards nonprefix sources and call kinds, but preserves uncovered explicit prefix", async () => {
	...
	prior.providerPayload = createOpenAIResponsesHistoryPayload(model.provider, [
		{ type: "custom_tool_call", call_id: "old", name: "read", input: "old" },
	]);
	const retained = bound({ role: "user" as const, content: "retained", timestamp: 1 }, "retained", 2);
	const snapshot = assistant([]);
	snapshot.providerPayload = createOpenAIResponsesHistoryPayload(
		model.provider,
		convertCodexResponsesMessages(model, { messages: [retained] }) as unknown as Record<string, unknown>[],
		false,
	);
	const after = bound(
		{
	...
	const context = {
		messages: [
			bound({ role: "user" as const, content: "discard", timestamp: 1 }, "discard", 0),
			prior,
			snapshot,
			after,
		],
	...
	const prefix = convertCodexResponsesMessages(model, {
		messages: [bound({ role: "user", content: "explicit", timestamp: 1 }, "prefix", -1)],
	});
	...
	expect(
		body.input!.some(item => item.type === "custom_tool_call_output" || item.type === "function_call_output"),
	).toBe(false);
	const wireInput = JSON.parse(JSON.stringify(body.input)) as InputItem[];
	const lateNotes = wireInput.filter(
		item =>
			(item.role === "user" || item.role === "assistant") && JSON.stringify(item.content).includes("late result"),
	);
	expect(lateNotes).toHaveLength(1);
	expect(JSON.stringify(lateNotes[0].content)).toContain("old");
	expect(JSON.stringify(wireInput)).not.toContain("discard");
});

test("computer replay unroll keeps original screenshot identity through Lite detail removal", async () => {
	const screenshot = setSourceOrigin(
		{ type: "computer_screenshot", image_url: "data:image/png;base64,aGVsbG8=" },
		{
			kind: "source",
			parts: [{ entryId: "shot", order: 1, blockIndex: 0, coverage: "full", representation: "original-image" }],
		},
	);
	const output = setSourceOrigin(
		{ type: "computer_call_output", call_id: "computer", output: screenshot },
		{
			kind: "source",
			parts: [
				{
	...
	carrier.providerPayload = createOpenAIResponsesHistoryPayload(
		model.provider,
		[
			setSourceOrigin(
				{ type: "computer_call", call_id: "computer", actions: [{ type: "click", x: 1, y: 2 }] },
				{
					kind: "source",
					parts: [{ entryId: "call", order: 0, blockIndex: 0, coverage: "full", representation: "native" }],
				},
			),
			output,
		],
	...
	expect(content.find(part => part.type === "input_image")).toMatchObject({
...
test("filtering, ID stripping, orphan truncation, missing results and Lite prefixes retain honest attribution", async () => {
	const origin = {
		...
		parts: [
			{ entryId: "tool", order: 1, blockIndex: 0, coverage: "full" as const, representation: "native" as const },
		],
	};
	const input: InputItem[] = [
		setSourceOrigin({ type: "item_reference", id: "drop" }, origin),
		setSourceOrigin(
			{ type: "function_call", id: "strip", call_id: "missing", name: "read", arguments: "{}" },
			origin,
		),
	...
	const body = await transformRequestBody(
		{ model: model.id, instructions: "base", input },
		model,
		{ responsesLite: true },
		{ developerMessages: ["extra"] },
	);
	...
	expect(getSourceOrigin(orphan)).toMatchObject({
		kind: "source",
		parts: [{ coverage: "partial", representation: "transformed-text" }],
	});
	for (const prefix of body.input!.filter(item => item.role === "developer"))
		expect(getSourceOrigin(prefix)?.kind).toBe("synthetic");
});
...
test("controlled local API receives raw in-place and replacement hooks with unknown attribution; response delta stays unbound", async () => {
	...
	try {
		...
		for (const mode of ["sse", "websocket"])
			for (const replacement of [false, true]) {
				let hooked: { input: InputItem[] } | undefined;
				const stream = streamOpenAICodexResponses(
					{ ...model, preferWebsockets: mode === "websocket", baseUrl: `http://127.0.0.1:${server.port}` },
					{ messages: [bound({ role: "user", content: "original", timestamp: 1 }, "u", 0)] },
					{
						apiKey: token,
						preferWebsockets: mode === "websocket",
						sessionId: `local-${mode}-${replacement}`,
				...
				expect(received.at(-1)!.input).toEqual([
					{ role: "user", content: replacement ? "replacement" : "mutated" },
				]);
				expect(result.providerPayload?.type).toBe("openaiResponsesHistory");
				if (result.providerPayload?.type === "openaiResponsesHistory")
					expect(sourceKeys(result.providerPayload.items)).toEqual([["unknown"]]);
			}
	} finally {
