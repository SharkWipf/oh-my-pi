...
import type { AssistantMessage, Context, Message, Model, ModelSpec, UserMessage } from "../src/types";
...
const fixtureModelSpec = {
	id: "source-origin-fixture",
	name: "Source origin fixture",
...
function serialize(
	model: Model<"openai-responses" | "azure-openai-responses">,
	messages: Message[],
	replay = false,
): ResponseInput {
...
describe("OpenAI source origins at actual serializers", () => {
	for (const model of [openai, azure]) {
		it(`${model.provider} retains block identity through one-to-many emission, text hoist and tool images without changing wire bytes`, () => {
			...
			expect(JSON.stringify(items)).toBe(
				JSON.stringify(serialize(model, JSON.parse(JSON.stringify(messages)), true)),
			);
			...
			if (!output || output.type !== "function_call_output" || !Array.isArray(output.output))
				throw new Error("Missing image result");
			expect(sourceParts(output.output[0])[0]).toMatchObject({
				entryId: "entry-2",
				blockIndex: 0,
	...
	it("imports prior full-snapshot maps and delta maps instead of attributing the snapshot to its carrier", () => {
		...
		full.providerPayload = JSON.parse(
			JSON.stringify(
				createOpenAIResponsesHistoryPayload("openai", original as unknown as Record<string, unknown>[], false),
			),
		);
		bindMessageSource(full, "carrier", 9);
		const replacement = serialize(openai, [...users, full], true);
		expect(replacement).toEqual(original);
		expect(replacement.map(item => sourceParts(item)[0].entryId)).toEqual([
			"historical-0",
			"historical-1",
			"historical-2",
		]);
		...
		delta.providerPayload = JSON.parse(
			JSON.stringify(
				createOpenAIResponsesHistoryPayload(
					"openai",
					convertResponsesAssistantMessage(delta, openai, 0, new Set()) as unknown as Record<string, unknown>[],
					true,
				),
			),
		);
		const appended = serialize(openai, [full, delta], true);
		expect(appended.map(item => sourceParts(item)[0].entryId)).toEqual([
			"historical-0",
			"historical-1",
			"historical-2",
			"delta-entry",
		...
		expect(exportItemOrigins(serialize(openai, [legacy], true)).every(origin => origin.kind === "unknown")).toBe(
			true,
		);
	});
	...
	it("transfers exact clone positions and treats escaped text, omitted images and synthetic repairs honestly", () => {
		const user: UserMessage = {
			role: "user",
			content: [
				{ type: "text", text: "<|channel|>analysis" },
				{ type: "image", data: "AA==", mimeType: "image/png" },
			],
			timestamp: 0,
		};
		bindMessageSource(user, "escaped", 0);
		const harmony = { ...openai, identity: { ...openai.identity, class: "gpt-oss" as const } };
		const item = escapeReplayedControlTokens(serialize(harmony, [user]))[0];
		expect(sourceParts(item)[0]).toMatchObject({
			entryId: "escaped",
			blockIndex: 0,
		...
		const result = setSourceOrigin(
			{ type: "function_call_output", call_id: "orphan", output: "x".repeat(17000) } as ResponseInput[number],
			{
				kind: "source",
				parts: [
					{
						entryId: "orphan-source",
						order: 2,
						blockIndex: 0,
						coverage: "full",
						representation: "native",
						sourceSpan: { start: 0, end: 17000 },
					},
				],
			},
		);
		expect(sourceParts(repairOrphanResponsesToolOutputs([result])[0])[0]).toMatchObject({
			entryId: "orphan-source",
			coverage: "derived",
			representation: "transformed-text",
		});
	...
	for (const baseModel of [openai, azure, createCodexModel("source-origin-fixture", { input: ["text", "image"] })]) {
		it(
			baseModel.provider +
				" escapes ordinary computer acknowledgments without changing screenshot identity or the non-Harmony request",
			() => {
				const marker = "<|channel|>analysis";
				const escaped = String.raw`<\|channel\|>analysis`;
				const model = { ...baseModel, supportsComputerUse: true };
				const call = assistant(model, [
					{
						type: "toolCall",
						id: "call_screen|item_screen",
						name: "computer",
						arguments: {},
						providerMetadata: {
							type: "computer",
							providerItemId: "item_screen",
				...
				const result: Message = {
					role: "toolResult",
					toolCallId: "call_screen|item_screen",
					toolName: "computer",
					content: [],
					isError: false,
					timestamp: 2,
					providerMetadata: { type: "computer", screenshot, acknowledgedSafetyChecks: [check] },
				};
				...
				expect(output).toMatchObject({
					call_id: "call_screen",
					output: screenshot,
					acknowledged_safety_checks: [{ ...check, message: escaped }],
				});
				expect(ordinary.find(item => item.type === "computer_call_output")).toMatchObject({
					output: screenshot,
					acknowledged_safety_checks: [check],
				});
				expect(sourceParts(output).find(part => part.blockIndex === "metadata.screenshot")).toMatchObject({
					entryId: "screen-result",
					coverage: "full",
					representation: "original-image",
				});
				expect(sourceParts(output.output)[0]).toMatchObject({
					entryId: "screen-result",
					blockIndex: "metadata.screenshot",
					representation: "original-image",
				});
				expect(
					sourceParts(output).find(part => part.blockIndex === "metadata.acknowledgedSafetyChecks.0"),
				).toMatchObject({ entryId: "screen-result", representation: "transformed-text" });
				...
				expect(sourceParts(replayedOutput.output)[0]).toMatchObject({
					entryId: "screen-result",
					blockIndex: "metadata.screenshot",
					representation: "original-image",
				});
		...
		for (const replacement of [false, true]) {
			it(`${model.provider} invalidates ${replacement ? "replacement" : "in-place"} hooks at the controlled HTTP boundary without reinjection`, async () => {
				...
				const options = {
					...
					azureBaseUrl: "https://offline.invalid/openai/v1",
					azureApiVersion: "v1",
					...
					fetch: async (_url: string | URL | Request, init?: RequestInit) => {
						...
						return new Response(JSON.stringify({ error: { message: "offline boundary stop" } }), {
							status: 400,
				...
				expect(prehook).toContainEqual(
					expect.objectContaining({
						kind: "source",
						parts: expect.arrayContaining([expect.objectContaining({ entryId: "protected-source" })]),
					}),
				);
