...
describe("issue #6913: Harmony control-token escaping at the request boundary", () => {
	...
	for (const api of ["openai-responses", "azure-openai-responses", "openai-codex-responses"] as const) {
		it("escapes typed native logical fields at the " + api + " builder boundary, not opaque payloads", () => {
			const provider =
				api === "azure-openai-responses" ? "azure" : api === "openai-codex-responses" ? "openai-codex" : "openai";
			const model = buildModel({
				id: "gpt-oss-120b",
				name: "Harmony fixture",
				api,
				provider,
			...
			const fixtures: Array<{ input: ResponseInput[number]; expected: object }> = [
				{
					input: {
						...
						pending_safety_checks: [{ id: opaque, code: opaque, message: MARKER }],
					},
					expected: {
						actions: [{ type: "type", text: ESCAPED }],
						pending_safety_checks: [{ id: opaque, code: opaque, message: ESCAPED }],
					},
				},
				{
					input: {
						...
						call_id: "computer",
						output: { type: "computer_screenshot", image_url: opaque },
						acknowledged_safety_checks: [{ id: opaque, code: opaque, message: MARKER }],
					},
					expected: {
						output: { type: "computer_screenshot", image_url: opaque },
						acknowledged_safety_checks: [{ id: opaque, code: opaque, message: ESCAPED }],
					},
				},
				{
					input: {
						...
						id: "ws_fixture",
						status: "completed",
						action: { type: "search", query: MARKER, queries: [MARKER], sources: [{ type: "url", url: MARKER }] },
					},
					expected: {
						action: {
							type: "search",
				...
				},
				{
					input: {
						...
						status: "completed",
						queries: [MARKER],
						results: [{ file_id: opaque, filename: MARKER, text: MARKER }],
					},
					expected: { queries: [ESCAPED], results: [{ file_id: opaque, filename: ESCAPED, text: ESCAPED }] },
				},
				{
					input: {
						type: "reasoning",
					...
					expected: {
						summary: [{ type: "summary_text", text: ESCAPED }],
						content: [{ type: "reasoning_text", text: ESCAPED }],
						encrypted_content: opaque,
					},
				},
				{
				...
				},
				{
					...
					expected: {
						name: opaque,
						server_label: opaque,
						arguments: JSON.stringify({ text: ESCAPED }),
						output: ESCAPED,
						error: ESCAPED,
					},
				},
				{
					input: {
						...
						id: "list_fixture",
						server_label: opaque,
						tools: [{ name: opaque, input_schema: { opaque }, annotations: { opaque }, description: MARKER }],
						error: MARKER,
					},
					expected: {
						server_label: opaque,
						tools: [{ name: opaque, input_schema: { opaque }, annotations: { opaque }, description: ESCAPED }],
						error: ESCAPED,
					},
				},
				{
					...
					expected: { name: opaque, server_label: opaque, arguments: JSON.stringify({ text: ESCAPED }) },
				},
				{
					input: {
						type: "mcp_approval_response",
						approval_request_id: "approve_fixture",
						approve: false,
						reason: MARKER,
					},
					expected: { approval_request_id: "approve_fixture", approve: false, reason: ESCAPED },
				},
				{
					input: { type: "shell_call", call_id: "shell", action: { commands: [MARKER] } },
					expected: { action: { commands: [ESCAPED] } },
				},
				{
					input: {
						type: "shell_call_output",
						call_id: "shell",
						output: [{ stdout: MARKER, stderr: MARKER, outcome: { type: "exit", exit_code: 0 } }],
					},
					expected: { output: [{ stdout: ESCAPED, stderr: ESCAPED, outcome: { type: "exit", exit_code: 0 } }] },
				},
				{
					input: {
						...
						call_id: "local",
						status: "completed",
						action: { type: "exec", command: [MARKER], env: { VALUE: MARKER }, working_directory: MARKER },
					},
					expected: { action: { command: [ESCAPED], env: { VALUE: ESCAPED }, working_directory: ESCAPED } },
				},
				{
					input: {
						type: "local_shell_call_output",
						id: "local_fixture",
						output: JSON.stringify({ stdout: MARKER }),
					},
					expected: { output: JSON.stringify({ stdout: ESCAPED }) },
				},
				{
					input: {
						...
						call_id: "patch",
						status: "completed",
						operation: { type: "create_file", path: MARKER, diff: MARKER },
					},
					expected: { operation: { type: "create_file", path: ESCAPED, diff: ESCAPED } },
				},
				{
					input: { type: "apply_patch_call_output", call_id: "patch", status: "completed", output: MARKER },
					expected: { output: ESCAPED },
				},
				{
					input: { type: "tool_search_call", arguments: { query: MARKER }, call_id: "tools", execution: "server" },
					expected: { arguments: { query: ESCAPED } },
				},
				{
					input: {
						...
						call_id: "tools",
						tools: [
							{ type: "function", name: opaque, parameters: { opaque }, strict: false, description: MARKER },
						],
					},
					expected: { tools: [{ name: opaque, parameters: { opaque }, description: ESCAPED }] },
				},
				{
					input: {
						...
						tools: [
							{
								...
								name: opaque,
								description: MARKER,
								tools: [{ type: "function", name: opaque, description: MARKER }],
							},
						],
					},
					expected: {
						tools: [{ name: opaque, description: ESCAPED, tools: [{ name: opaque, description: ESCAPED }] }],
					},
				},
				{
					input: { type: "image_generation_call", id: "image_fixture", status: "completed", result: opaque },
					expected: { result: opaque },
				},
			];
			const items = fixtures.map(({ input }) => input);
			for (const item of items)
				setSourceOrigin(item, {
					kind: "source",
					parts: [
						{
							entryId: "source-" + item.type,
							order: 0,
							blockIndex: 0,
			...
			const convert = (activeModel: typeof model) =>
				api === "openai-codex-responses"
			...
			for (const fixture of fixtures) {
				...
				if (fixture.input.type === "web_search_call")
					expect(getSourceOrigin(item!)).toMatchObject({
						kind: "source",
						parts: [{ entryId: "source-web_search_call", coverage: "full", representation: "transformed-text" }],
					});
			}
