...
export interface ContextUsageSession extends NonMessageTokenSource {
	getCompactionDiagnostics?(snapshot: "recorded"): CompactionDiagnostics | undefined;
...
export function computeNonMessageBreakdown(
	...
	const tools = session.agent?.state?.tools ?? EMPTY_TOOLS;
	const visibleSkills = skillful === false ? EMPTY_SKILLS : renderedSkills(session.skills ?? EMPTY_SKILLS, tools);
