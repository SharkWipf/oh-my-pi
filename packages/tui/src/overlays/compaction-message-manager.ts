...
const HELP = [
	...
	"Filters: Space toggles the focused role or stored-state checkbox; y includes and n excludes it. Stored Auto is a boolean visibility filter, not a third checkbox state. Text input owns all keys while search is active.",
...
export class CompactionMessageManagerComponent implements Component {
	...
	#menuStart = 0;
	#filterRows: Map<string, { included: boolean; set(included: boolean): void }> | undefined;
	...
	#filters(selected = 0): void {
		...
		const rows = new Map<string, { included: boolean; set(included: boolean): void }>();
		for (const role of ROLES) {
			const value = `role:${role}`;
			const included = this.#roles.has(role);
			items.push({ value, label: `${included ? "[x]" : "[ ]"} Role: ${role}` });
			rows.set(value, {
				included,
				set: enabled => {
					if (enabled) this.#roles.add(role);
		...
		for (const state of STATES) {
			...
			const included = this.#states.has(state);
			items.push({ value, label: `${included ? "[x]" : "[ ]"} Stored: ${state}` });
			rows.set(value, {
				included,
				set: enabled => {
					if (enabled) this.#states.add(state);
	...
	#setFilter(value: string, included?: boolean): void {
		const row = this.#filterRows?.get(value);
		if (!row) return;
		const next = included ?? !row.included;
		if (next === row.included) return;
		const selected = [...this.#filterRows!.keys()].indexOf(value);
		row.set(next);
		this.#filter();
	...
	handleInput(data: string): void {
		...
		if (this.#menu) {
			...
			if (selected && this.#filterRows?.has(selected.value) && (data === " " || data === "y" || data === "n")) {
				this.#setFilter(selected.value, data === " " ? undefined : data === "y");
		...
		if (this.#document) {
			...
			else if (data === "l" && this.#document.sourceId) this.#sourceSettings(this.#document.sourceId);
			this.#requestRender();
			return;
		} else {
	...
	render(width: number): readonly string[] {
		...
		const controls = this.#input
			...
			: this.#menu
				? [
						[this.#filterRows ? "Space toggle · y include · n exclude · Enter action" : "Enter select", ""],
						["Esc Back", "back"],
					]
				: this.#document
					? [
					...
					: [
							...
							["? Help", "help"],
							["Esc Back", "back"],
							["Space/Enter cycle · y/* Always · n Never · -/Backspace Auto", ""],
							["/ Search", "search"],
							["f Filters", "filters"],
							["i Inspect", "inspect"],
						];
		for (const [label, action] of controls) {
			...
			if (start) footer += " · ";
			if (action)
				this.#footerHits.push({ start: start ? start + 3 : 0, end: footer.length + label!.length, action });
			footer += label;
		}
