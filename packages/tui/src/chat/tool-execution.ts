...
export class ToolExecutionComponent extends Container {
	...
	#rebuildDisplay(): void {
		...
		if (benignSkip) {
			...
			const details = this.#result?.details;
			const perFileResults = (isRecord(details) ? details.perFileResults : undefined) as
			...
			if (perFileResults && perFileResults.length > 1) {
				...
				const edits = isRecord(this.#args) ? this.#args.edits : undefined;
				const totalFiles = Array.isArray(edits)
					? new Set(edits.map((entry: unknown) => (isRecord(entry) ? entry.path : undefined)).filter(Boolean)).size
					: 0;
