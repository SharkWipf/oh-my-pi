...
Exact-source retry, adoption and evidence review do not wait for unrelated cold catalog enumeration; normal evidence revalidation and publication checks still apply. Broad backfill remains explicit, and unobserved catalog coverage is not promoted to complete merely because current requirements are available. Completing catalog observation refreshes future coverage receipts without changing earlier receipts or active requirements.
...
File-backed original reads use a lazy journal entry-location index rather than repeatedly decoding the whole journal. Normal appends and title updates maintain those locations; external changes and rewrites invalidate them. Each evidence read still freshly decodes the complete addressed records and journal identity. Unchanged source intake updates metadata without expanding historical review dependencies; genuine original changes and availability restoration retain full affected-dependency reconciliation. Contextual source metadata is installed before a review job freezes its read set, so discovering an original does not invalidate that same job.
