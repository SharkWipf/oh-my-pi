...
- Original submitted text and image bytes are reconstructed as an editable draft, retaining `/keep` or `/once` disposition and positional image links rather than provider-normalized images
- An unchanged restored draft reuses its original source; editing it supplies a new original on submission
- A referenced original that is unavailable prevents navigation; older messages without originals restore their stored text and images normally
