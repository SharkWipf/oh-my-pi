...
- **Live reset details in the usage CLI**: `omp usage --provider openai-codex --reset-credits --json` fetches saved credits and the full paginated reset history directly from the client, independently of the broker quota projection. Matching quota reports receive `resetCredits` details only through an explicit account ID; the top-level `resetCreditAccounts` array also retains accounts without quota reports and identifies each with `provider: "openai-codex"`. Credit-list failures retain any broker count but omit stale credit details and expose `creditsError`; history completeness, fetch time, window/as-of bounds, and `historyError` remain independent. A global reset fetch failure preserves quotas and exposes `resetCreditsError` instead of an authoritative empty account list. `--redact` also masks the new account/email fields. The flag does not redeem credits, does not change ordinary usage without the flag, and is ignored for `--history`, `clients`, `invalidate`, or a non-Codex provider filter.
- **Reset history persistence and display**: Ordinary cached usage reports retain every observed credit status and identifier, including redeemed or expired credits when no resets remain available. The optional paginated reset-event history is live CLI enrichment only: it is exposed in `--reset-credits --json`, not written back to the usage cache. Human-readable usage views continue to show available reset counts and expiry dates, not a historical event ledger.
- **Reset collection deadline**: Saved-reset account access, credit lists, and all history pages share the existing usage-request timeout (10 seconds by default) and caller cancellation. Accounts and the two detail routes run concurrently; a timed-out optional route leaves quota reporting and independently completed details intact.
